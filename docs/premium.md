...
- **Gate `/scrapbook` with `requirePremium`**, deferred. Today scrapbook is free-tier Workers AI and anonymous-first, so gating it would break the free monthly keepsake and guards no real money. Trigger: scrapbook moves to a paid image model, OR the D1 telemetry shows abuse.
