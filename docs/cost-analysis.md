...
- **Apple, when iOS ships. THE DECISION IS MADE AND THIS MODEL IS NOW STALE.** As of 2026-07-15, Apple IAP ships from iOS v1 (see `decision-log.md`), reversing the old plan of hiding every purchase path under 3.1.3(b). So on iOS-sourced subscriptions **Apple's 15% (Small Business Program) replaces Stripe's ~2.9% + $0.30**, a materially different fee structure that the rows below do NOT include. Parity pricing means the cut is absorbed, not passed on, so iOS revenue nets ~12% less per subscription than the same sale on web. Re-cut the Stripe rows before quoting any blended margin.
