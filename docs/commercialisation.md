...
**The Apple decision, pre-named.** Premium today is Stripe, and an App Store release cannot ship until the In-App-Purchase strategy is decided, Apple's rules make that a gate, not a detail. The pre-decided path (recorded in `BUILD-PLAN.md`): the first iOS build hides every purchase path, relying on Apple's 3.1.3(b) allowance that features bought elsewhere may unlock for signed-in users so long as iOS never steers to external purchase, and real IAP (via RevenueCat, accepting Apple's 15% cut) comes only when iOS revenue justifies it. Pre-deciding this now is deliberate: the awkward call is made before launch pressure, not during it.
