...
| TOD-07b | P2 | Both | First-run coachmark teaches the long-press | On a device that has never seen it (or after clearing 'doubledone.holdhint.v1'), open Today with at least one task. Read the hint, tap 'Got it', then reload. Separately check an empty Today and the rested (closed-day) screen. | A calm one-line hint sits above the list: 'Hold a task for more: pin it, set a reminder, break it down, or make it tiny.' 'Got it' dismisses it for good (survives reload). It never shows on an empty Today or the rested screen, and never returns once dismissed. |
| TOD-07c | P2 | Both | Split a task into steps (and resize or make whole) | Tap-and-hold a single task, then tap 'Steps' in the action bar. With the - / + stepper pick a number of parts (2-50) and tap Done. Tap the task on Today to advance a step. Hold it again, 'Steps', change the count, Done. Then hold, 'Steps', 'Make it whole again'. | 'Steps' opens a 'Track in steps' editor with a stepper bounded at 2 and 50. Done turns the task into an N-part one with a step counter; tapping it on Today advances one step, and finishing the last step completes it (with the whole-task bloom). Re-opening lets you change N: progress carries over, and shrinking below what's already done snaps it down. 'Make it whole again' drops the parts back to one task, keeping its done state. Single-task, non-recurring, not-done only. Free for everyone. |
...
| TOD-20b | P2 | Both | Combine WITHOUT AI (name it yourself) | With AI off, long-press to select two or more eligible tasks and tap 'Combine'. | 'Combine' appears with AI off just as with AI on. The review opens with the name PREFILLED from the selected titles joined by commas (no AI call, no name-finding step), editable. On accept the originals fold into one umbrella exactly as the AI path does (earliest due date, originals recorded for un-combine, select mode exits). Only the source of the suggested name differs. |
...
| RHY-02 | P2 | Both | Make a custom Rhythm, and edit a preset (fully flexible) | Tap '+ New rhythm'. Name it (e.g. Stretch), set 'How often' with the stepper (every 1-12 hours) and the 'Active hours' window (start / end), watch the live cadence preview, tap 'Add rhythm'. Then tap Edit on the Water card, change its interval, window or name, and Save changes. | The form creates a custom Rhythm with the chosen name, interval and window, and the card shows the matching cadence line. A missing name focuses the name field with a calm hint, never a silent bounce. Editing a preset opens the same form prefilled and updates it in place, so the presets are starting points, not fixed. Any change reschedules immediately. Still no checkboxes, no progress, no streak. |
...
| ID | Pri | Platform | Test | Steps | Expected |
|---|---|---|---|---|---|
| REP-01 | P1 | Both | Removing a repeating task from Today skips ONLY today | Long-press a due repeating task on Today, tap Remove (single and via multi-select). | Today manages days: the instance vanishes from Today with 'Skipped just for today. The repeat continues.', the series survives (still in the Repeating drawer), and it returns on its next due day. A mixed selection tombstones the one-offs and skips the recurring. Screen readers hear 'Remove today's instance of {title}'. |
...
| ID | Pri | Platform | Test | Steps | Expected |
|---|---|---|---|---|---|
| TOD-23 | P2 | Both | Done on… corrects a COMPLETED task's day | Tap a rolled-over task done (it records today). Long-press the completed task: the bar's Done action is replaced by 'Done on…'. Pick a past day (yesterday to 14 back; today is not offered). Also long-press an OPEN task and confirm 'Done on…' is NOT offered there. | Correction is a property of a completed task: the natural flow is tap-done first, refine after. Picking a day re-stamps the completion to that day's noon; the Lookback attributes it honestly on every device; the affirm reads 'Recorded for {day}. Your Lookback tells it true.', no bloom, no haptic. Recurring tasks never offer it. |
...
| QUI-02 | P2 | Both | Switching Standard <-> Quiet never moves the layout | With a handful of tasks on Today (some open, one or two done), toggle Interface between Standard and Quiet a few times, watching the task rows and the capture line. | Content stays put: no visible jump in row height or position as you toggle. Quiet removes chrome (row borders, fills, the per-task progress bar, the capture underline weight) but preserves spacing, so nothing shifts under the finger. This is the promise the design makes ('switching never moves anything'). |
| QUI-03 | P2 | Both | Quiet held-state: long-press reveals the row's actions in place | In Quiet, press and hold a one-off task on Today, then also hold a task in the 'Later' list and (if you have one) a recurring task. | The held row gets a soft wash and its actions appear inline ON the row, at full parity with Standard's select bar for a single task: Tomorrow, Mark as a lot (Big), Make it tiny, Break down, Pin, Select more, Remove, Close. Mark-as-a-lot and Pin act-and-dismiss (the row updates, the held-state closes). Pin shows only on Today one-off rows (not Later, not recurring), matching PIN-05; a lapsed subscriber still on Quiet who taps Pin is routed to the paywall, never wrongly pinned. 'Select more' closes the held-state and drops into the normal multi-select with THIS task pre-selected ('1 selected'), which is Quiet's one path to the bulk actions that need 2+ tasks (Combine, move several to a day, tick several off); Combine appears once a second task is selected. No held-state (single-row) select bar appears otherwise. Standard is unchanged: its long-press still opens the multi-select bar. The one-time 'hold a task for more' coachmark in Quiet is a faint line with a small accent dot, not a filled pill. |
