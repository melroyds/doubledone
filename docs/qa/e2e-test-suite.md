...
| CAP-09 | P2 | Both | Split a run-on into separate tasks (AI) | Type or speak a single run-on line of several things ('buy milk and walk the dog and email Sarah'), then tap 'More than one thing in there? Split into tasks'. | The line is replaced by the separate tasks, one per line, in your own words (nothing invented or reordered), and 'Sort for me' then appears to sort them. On failure it degrades calmly ('Couldn't split that just now'), text kept. Works on web and Android. Needs the Worker deployed with /split. |
