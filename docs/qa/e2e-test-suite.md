...
| QUI-03 | P2 | Both | Quiet held-state: long-press reveals the row's actions in place | In Quiet, press and hold a one-off task on Today, then also hold a task in the 'Later' list and (if you have one) a recurring task. | The held row gets a soft wash and its actions appear inline ON the row, at full parity with Standard's select bar for a single task: Tomorrow, Mark as a lot (Big), Make it tiny, Break down, Pin, Remove, Close. Mark-as-a-lot and Pin act-and-dismiss (the row updates, the held-state closes). Pin shows only on Today one-off rows (not Later, not recurring), matching PIN-05; a lapsed subscriber still on Quiet who taps Pin is routed to the paywall, never wrongly pinned. NO full-screen select bar appears. Standard is unchanged: its long-press still opens the multi-select bar. The one-time 'hold a task for more' coachmark in Quiet is a faint line with a small accent dot, not a filled pill. |
