...
CASES = [
    ...
    ("CAP-10", "Capture", "P1", "The one door: weekly end to end",
     "Tap the line at the bottom of Today ('Empty your head…'), type 'water the plants', tap the 'WHEN Today ˅' chip, tap Weekly, then Add.",
     "The When chip opens the door in place of the keyboard: 'A day' (Today / Tomorrow / Pick a date), 'Or a rhythm' (Daily / Weekly / Every few days / Monthly) and Steps, with the summary top right. The summary reads 'Today · Weekly on <today's day>' and the Add button reads 'Add · Weekly on <day>' BEFORE the tap. After Add the task lands with the repeat glyph, the door resets to Today, and the cursor stays in the box for the next line. Weekly is 3 taps beyond typing, never more.", "Both"),
    ("CAP-11", "Capture", "P1", "Text is never lost; the page comes back when you let go",
     "Tap the line at the bottom, type half a thought, then tap somewhere else on the page (a bare patch of background) so the box lets go. Tick a task. Tap the box again.",
     "While the box has focus, Today's heading shrinks to a compact 'Today' bar and Right now, Energy and the greeting step aside. The moment the box lets go they all come back, and the half thought waits in the box, calmly, without holding the page open. Tapping the box again shows the text still there. Nothing typed is ever lost by the box closing.", "Both"),
    ("CAP-13", "Capture", "P1", "The composer rides ABOVE the keyboard, never under it",
     "On a real Android phone (app and browser separately), tap the line at the bottom and watch where it lands as the keyboard rises. Type a line, tap the When chip, pick Weekly, tap the box again, Add. Repeat at a large font-size setting.",
     "The whole composer (the When chip row, the box, Add) sits above the keyboard at all times; you can see what you type and reach Add without dismissing the keyboard. Opening When puts the keyboard away and shows the choices; tapping the box brings the keyboard back and folds When to its chip. REGRESSION GUARD: a tester's v19 screenshots showed only 'ADD / Close' with everything else under the keyboard, because edge-to-edge Android ignores adjustResize and Chrome 108+ overlays the keyboard by default; the fix is the keyboard-height footer lift (native) + interactive-widget=resizes-content (web).", "Android"),
    ...
    ("TOD-32", "Today", "P2", "The composer on a wide browser window",
     "On a desktop browser at full width, open Today, click the line at the bottom, type, open When, pick Tomorrow, Add.",
     "The composer sits centred under the list at the same width as the list, docked to the bottom. The cursor stays in the box after Add and does not drop out a second later (a regression found in the preview: the page's own scroll to a just-added row used to put the keyboard away on web). The browser's own focus rectangle never shows inside the rounded box; the box's accent edge is the focus sign.", "Web"),
    ...
    ("CAP-14", "Capture", "P1", "Share TEXT into DoubleDone on iPhone (1.5.1 or later)",
     ...
     "DoubleDone is in the sheet for BOTH a text selection and a page URL, and opens with the capture box on Today OPEN, focused, and prefilled with one calm line (words kept, links dropped when words exist; a bare link kept with its highlight fragment stripped), cold and warm alike. Nothing is added until you tap Add. On 1.5.0 a text share never listed DoubleDone at all: the extension's default activation rule was URL-only (found 2026-09-25, fixed in app.json), so on 1.5.0 only the Safari URL share shows it.", "iOS"),
    ("CAP-15", "Capture", "P1", "The composer is one line until you use it (Today v3)",
     "Open Today and look at the bottom. Tap the line. Type something. Then tap outside.",
     "At rest the bottom of Today is ONE line: Scan (with AI on), a rounded box reading 'Empty your head…', and Speak (web) inside it. No launcher, no second tap. Tapping it opens it where it is: the keyboard rises, the heading shrinks to a compact bar (the word 'Today', the weight label, a thin gauge), a chip row appears above the box ('WHEN Today ˅' and, with AI on, Break it down or Sort for me), and the list scrolls to its end so the newest row is in view. Speak turns into Add the moment there is text. Tapping outside puts it all back. On a phone there is never a moment where two bottom bars compete.", "Both"),
    ("CAP-16", "Capture", "P1", "Add keeps you composing and marks what landed",
     "Tap the line, type 'call the plumber', Add. Type 'water the fern', Add. Then tap outside. Repeat once with When set to Tomorrow, and once with a screen reader on (VoiceOver / TalkBack / a browser screen reader).",
     "After each Add the cursor stays in the box, so the next thing goes straight in. Each row added on Today carries a warm tint and a small 'just added' mark for as long as you keep composing, so you can see where it went without looking for it. Tapping outside clears the marks (a short fade, none under reduced motion). A screen reader hears 'Added.' after an ordinary add and 'Added. Tomorrow.' when it went somewhere else, on web as well as on the phone. A Tomorrow add lands under Later, not on Today.", "Both"),
    ("CAP-17", "Capture", "P2", "Break it down keeps your words until you accept",
     "AI on. Tap the line, type a big task ('sort out the garage'), tap Break it down. When the proposal appears, dismiss it. Then do it again and accept.",
     "Dismissing the proposal leaves 'sort out the garage' in the box, untouched, so nothing typed is lost to a change of mind. Accepting adds the steps and only then clears the box.", "Both"),
    ...
    ("OUR-124", "Ours", "P2", "The room opens with your list's own name, in any language",
     ...
     "The room's heading shows your list's own name from the first frame, and the composer reads 'Aggiungi a Just us'. It never shows the default ('La nostra lista') for a beat and then flips, which read as the app changing language. A list with no name shows the default in the app's language. The list's own name is shown as you typed it, in whatever language that was.", "Both"),
    ...
    ("OUR-55", "Ours", "P1", "Capture on Ours IS Today's composer",
     "Open a shared list and tap the line at the foot of the screen ('Add to {name}…').",
     "The same composer Today has: the same box, the same Speak button (web), the same When chip and door, the same Add button naming its own consequence. Not a lookalike, the same component, so the two can never drift apart. It is one line at rest, with no launcher.", "Both"),
    ...
    ("OUR-59", "Ours", "P2", "Typed text survives the box letting go",
     "Type half a sentence into the Ours composer, tap somewhere else on the page, tick something, then tap the box again.",
     "The half sentence is still there. The composer is always mounted, never unmounted. Only an Add resets the door; the words never reset on their own.", "Both"),
    ("OUR-60", "Ours", "P2", "The Ours composer rides above the keyboard",
     "On a real Android device, tap the line at the bottom of the Ours list and start typing.",
     "The composer and its Add button stay visible above the keyboard, and the room's heading shrinks to a compact bar while you type. Nothing on this stack does this by itself (edge-to-edge Android ignores softwareKeyboardLayoutMode), so it is done by hand and must be re-checked on a device, never only in a browser.", "Android"),
    ("OUR-61", "Ours", "P3", "No AI on the shared capture",
     "Open the Ours composer with AI enabled in Settings, and type a long run-on line.",
    ...
    ("OUR-71", "Ours", "P1", "The shared capture rests on Anytime, and says what a day means",
     "Tap the Ours composer and read the line beside the When chip. Open the door. Pick Today, then Tomorrow, then Daily, reading that line each time.",
