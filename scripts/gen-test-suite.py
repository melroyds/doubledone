...
CASES = [
    ...
    ("TOD-20b", "Today", "P2", "Combine WITHOUT AI (name it yourself)",
     "With AI off, long-press to select two or more eligible tasks and tap 'Combine'.",
    ...
    ("RHY-02", "Rhythms", "P2", "Make a custom Rhythm, and edit a preset (fully flexible)",
     "Tap '+ New rhythm'. Name it (e.g. Stretch), set 'How often' with the stepper (every 1-12 hours) and the 'Active hours' window (start / end), watch the live cadence preview, tap 'Add rhythm'. Then tap Edit on the Water card, change its interval, window or name, and Save changes.",
     "The form creates a custom Rhythm with the chosen name, interval and window, and the card shows the matching cadence line. A missing name focuses the name field with a calm hint, never a silent bounce. Editing a preset opens the same form prefilled and updates it in place, so the presets are starting points, not fixed. Any change reschedules immediately. Still no checkboxes, no progress, no streak.", "Both"),
    ...
    ("RHY-05", "Rhythms", "P1", "Rhythm nudges actually arrive, visibly, and the health line tells the truth",
     "On Android with at least one active Rhythm: check the faint lines under the Rhythms section ('Set on this phone: N. Next around {time}.'), wait for the next slot, and confirm a nudge PEEKS (heads-up banner, not just a silent tray item). Then force-close and reopen the app and re-check the count. If nudges still do not arrive, tap 'Open app settings' and allow unrestricted battery use, then re-test.",
     "The health line's count matches the expected slots (all active Rhythms + checklist nudges + the daily reminder) and the next time is right. Nudges arrive on their own HIGH-importance 'Rhythms' channel with a heads-up peek (the old silent DEFAULT channel was the 'I only got one alarm' bug), still delivered 'around' the time, never exactly. Reopening the app re-schedules everything quietly (the resilience sweep), so the count never decays after an update, reboot, or an OEM battery wipe, and the sweep NEVER shows a permission prompt. Zero-count while Rhythms exist is a red flag the line itself surfaces. Battery-managed phones (Samsung/Xiaomi/OnePlus) may still delay nudges until the app is excluded from battery limits, which is what the settings link is for.", "Android"),
    ...
    ("TOD-23", "Today", "P2", "Done on… corrects a COMPLETED task's day",
     "Tap a rolled-over task done (it records today). Long-press the completed task: the bar's Done action is replaced by 'Done on…'. Pick a past day (yesterday to 14 back; today is not offered). Also long-press an OPEN task and confirm 'Done on…' is NOT offered there.",
    ...
    ("AND-04", "Android", "P2", "Share text into DoubleDone (Android sheet + installed web app)",
     ...
     "Both paths behave identically: DoubleDone opens with the shared text (or URL) already in the capture box on Today, OPEN, focused and ready to add, including when the capture box was collapsed (a share must never be silently dropped). The precedence is text, then URL, then title, one line. Adding it makes a normal task; nothing is sent anywhere until you do. The web path needs the PWA installed (that is how browsers expose share targets); an uninstalled tab simply has no DoubleDone entry on the sheet.", "Both"),
    ...
    ("AI-01b", "AI decompose", "P1", "Break it down WITHOUT AI (type the steps yourself)",
     "With AI off (Settings, AI, Turn AI off), long-press a task (or select it) and tap 'Break it down'.",
    ...
    ("QUI-03", "Interface", "P2", "Quiet held-state: long-press reveals the row's actions in place",
     "In Quiet, press and hold a one-off task on Today, then also hold a task in the 'Later' list and (if you have one) a recurring task.",
