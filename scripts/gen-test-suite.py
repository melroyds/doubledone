...
CASES = [
    ...
    ("TOD-10", "Today", "P2", "Focus mode: pick-and-go",
     ...
     "Full-screen single task, everything else hidden. Done completes it and returns to 'Which one?'; Choose another returns without completing; Exit closes. When none left: 'That's everything for now.'. With a LONG Today the picker SCROLLS and every row is reachable, including the last, while Exit stays pinned in its corner and never scrolls away. Regression guard: the picker used to be a centred non-scrolling column, so a list taller than the screen was clipped at BOTH ends and those tasks could not be selected at all (found 2026-07-25).", "Both"),
