...
CASES = [
    ...
    ("SETL-01", "Settle", "P1", "The breathing room: rhythm, words, and the never-rules",
     ...
     "The room holds ONLY: Leave (top-left), the breathing blob, one word slot, and the guide pill (reads 'Breathing guide · on/off'). NO title, NO timer or duration anywhere, NO stats, NO paywall or lock (free, unlimited, forever). The blob swells ~4s, holds ~1.5s, settles ~6.5s, on a loop, BRIGHTENING as it fills (an inner glow blooms at the top of the in-breath) and dimming as it empties; guide ON fades the three literal words in and out in phase ('in, gently…' / 'rest here' / 'slowly out…'), each breath wearing the next Dusk accent like Today's inscription, never a hard swap. Guide OFF: words stop, and at most one affirmation appears every 60-90s, fading in over ~3s and gone within 12, never stacking, never praising. The toggle is REMEMBERED across visits. Leaving fades out on the out-breath (≤800ms) with 'Today is where you left it.', then Today exactly as left: no summary, no toast.", "Both"),
