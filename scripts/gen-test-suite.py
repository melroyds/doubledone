...
CASES = [
    ...
    ("OUR-35", "Ours", "P1", "Reopening together is a handshake, never unilateral",
     "On a closed list, tap 'Reopen together\u2026' on ONE side only. Read the code to the second account. On that side, tap 'Got a code from them? Enter it to reopen.' (under the reopen hint on the closed-list screen) and enter it. Repeat with the closed list being the ONLY closed list on both accounts.",
     "The offering side gets a code and nothing reopens yet. The other side has a named door to the join field and entering the code there reopens the list for both, with everything as it was. It is never asked which kind of code it is, and it works when the closed list on screen is the only one (the D2 'handshake purgatory', 2026-09-22).", "Both"),
