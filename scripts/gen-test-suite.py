...
CASES = [
    ...
    ("TOD-30", "Today", "P2", "Repeating from the Menu comes back to the Menu",
     "Tap Menu, then the Repeating card. Close the Repeating drawer. Then open Repeating another way (tap a repeating row's glyph, or any other door to it) and close it.",
     "The Menu's Repeating card lands on Today with the Repeating drawer already open over it. Closing the drawer returns you to 'The rest of the house', where you started, not to a Today you never asked for. Opened any other way, closing it leaves you on Today as before.", "Both"),
    ...
    ("TOD-35", "Today", "P2", "Repeating from the Menu has the same room top",
     "Open Menu › Repeating. Read the top of the drawer, scroll the list, then close it with '‹ Menu' (and, on Android, with the system back). Repeat with a screen reader on.",
     "The drawer's top is '‹ Menu' (clear of the status bar on a notched phone), then the washing-line picture as a band, 'Repeating' in the serif and 'For the bins, the bills, the plants', then the not-a-notification line. The band and title scroll away with the list; '‹ Menu' stays. '‹ Menu' and Android's back both close it and return to the contents page, and the top never flickers to a different button as it slides out. A screen reader lands on the 'Repeating' heading when it opens, and hears nothing of the drawer while it is closed.", "Both"),
    ("TOD-36", "Today", "P1", "A done task wears one sign: the accent tick",
     "Tick a task on Today, a stepped task to its last step, a routine step, and a repeat in the Repeating drawer. Try two colour themes, light and dark.",
    ...
    ("REP-05", "Repeating", "P1", "Monthly: capture a task on a day of the month",
     ...
     "Monthly sits as a fourth chip beside Daily / Weekly / Every few days, and opens a calendar-shaped grid of 1 to 31 with TODAY'S day already chosen (so the common case is one tap, not two). The Add button names the outcome before the tap: 'Add · Monthly on the 15th'. The task lands repeating, appears in the Repeating drawer as 'Every month on the 15th', and reaches Today on that day of every month.", "Both"),
    ...
    ("MCP-18", "MCP", "P1", "complete_task on a REPEATING task ticks one day, never closes it",
     "With a daily repeat on Today (it shows as (repeats) in list_today): ask the agent to complete it. Ask again. Then open the app (cold), check Today, the Repeating drawer and the Lookback. Then ask the agent to complete it for today's LOCAL date (say the date).",
     "First: 'Ticked for <date>. It repeats, so it will be back on its next day.' Second: 'Already ticked for <date>...'. In the app the task is still in the Repeating drawer, comes back on its next day, and the tick shows on <date> in the Lookback. Before about 10am AEST <date> is YESTERDAY's (the UTC day, named in the answer, known); the agent passing `day` with the local date lands it on today. It never ends the series: the repeat is still there tomorrow.", "Desktop"),
    ...
    ("OUR-117", "Ours", "P2", "The Repeating drawer is untouched by the When editor",
     "Menu -> Repeating -> Edit on any series. Compare with before.",
     "Same sheet as always: title, the four cadence chips (Daily / Weekly / Every few days / Monthly), weekday toggles or the stepper or the 1-31 grid, 'Starting from' with its date, and a commit button naming the cadence. NO Anytime chip, NO day zone, and tapping the selected cadence chip does NOT release it. 'No rhythm' there would mean the entry should not exist, which is removal, and removal stays in the drawer.", "Both"),
    ("OUR-119", "Ours", "P2", "Monthly works on a shared row, and an out-of-date partner still sees the rhythm",
     ...
     "Monthly is the fourth rhythm chip here too, from the same sheet the personal drawer uses, so a shared monthly row and a personal one are identical in shape. The summary reads 'Every month on the 15th · from {start}' and the commit button names it. The row lands on BOTH Todays on that day. A partner whose build predates monthly cannot compute the days, so their app shows the row with the plain-English line the writing client left alongside the rhythm rather than showing a repeating row with no rhythm at all. What it must never do is HIDE the row: one person seeing a task the other does not is the invisible disagreement this whole feature exists to prevent.", "Both"),
