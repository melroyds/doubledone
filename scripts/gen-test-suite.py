...
CASES = [
    ...
    ("TOD-16", "Today", "P1", "The Menu pill opens 'The rest of the house' (Today v3)",
     "On Today, tap the Menu pill top right. Read the page. Tap each card in turn and come back. Try it signed out, signed in with no shared list, and with a live list. Try with AI off, and with Premium and without.",
     "The header shows the date and one pill (three dots and 'Menu'; Quiet appearance keeps just the word). It opens a PAGE, not a sheet, fading and rising in: '‹ Today' top left, Settings top right, the title 'The rest of the house' and the lead 'Today is the kitchen. These are the other rooms.' A wide Calendar card with its picture comes first ('For when your brain says you did nothing'), then a grid: Routines, Repeating, Ours and Chart a course, each with a picture and a for-when line. Chart a course is there only with AI on, and wears the honey ✦ when you are not Premium. Ours reads 'A list kept with one person. Sign in first.' when signed out and goes to sign-in; signed in it goes to the pairing screen, or straight to the list when one is live. A quiet Premium row closes the page. '‹ Today' returns to Today. The greeting under 'Today' still changes with the clock ('Good morning. Just today.', 'Winding down. Just today.').", "Both"),
