...
CASES = [
    ...
    ("OUR-120", "Ours", "P1", "Waiting-with-no-code says so, and the re-mint is a real button",
     "Mint a code, leave Ours entirely (back to Today), then come back to Ours before anyone joins.",
     "The screen is titled 'Waiting for your person', NOT 'Read this to your person' over an empty space (the code renders exactly once; the server keeps only a hash, so it can never be re-shown). 'Get a new code' is a full primary button in this state, not a quiet text link (the 2026-09-13 field report walked right past the link: 'no easy way to... regenerate the code'). While the code IS still on screen, the title and the quiet link stay as they were.", "Both"),
