...
CASES = [
    ...
    ("TOD-26", "Today", "P2", "Held-card action labels render in full, never a clipped glyph",
     "On a DENSE Android device (Samsung S22, 3x density, is the known repro) hold a task and read the bottom action row: 'Select more' (when shown), 'Remove', 'Close'. Repeat with the system font size bumped up, and in a longer locale (fr 'Retirer', it 'Rimuovi').",
     "Every label renders whole, with no trailing glyph clipped ('Remove' never reads 'Remov'). 'Remove' and 'Close' sit as one content-sized group in the thumb's corner (Close rightmost, on the accent); 'Select more' stays faint and left. Regression guard for the fractional flex-spacer that starved the middle label's box on an S22 (reported 2026-07-28): each label is now content-sized, never measured from leftover space.", "Android"),
    ...
    ("AND-05", "Android", "P1", "Home-screen widget renders Today (re-enabled + self-diagnosing)",
     "On Android, long-press the home screen, open the widget picker, and place the DoubleDone 'Today' widget. Read what it draws. Then add or complete a task in the app and re-check the widget after a moment (it also self-updates every ~30 min).",
