...
CASES = [
    ...
    ("CAP-01", "Capture", "P1", "Add a single task",
     "On Today, tap the + pill at the foot of the screen, tap the box, type 'Buy milk', tap Add.",
     "The task appears in Today immediately, just above the panel. No reload needed.", "Both"),
    ...
    ("CAP-07", "Capture", "P2", "Talk-to-capture: speak a brain-dump (web)",
     "On the web app in Chrome / Edge / Safari, tap the + pill, then '🎤 Speak' in the panel's tools row and allow the mic. Say a few tasks, pausing between each, then tap 'Listening… tap to stop'.",
    ...
    ("CAP-10", "Capture", "P1", "The one door: weekly end to end",
     "Tap the + pill at the foot of Today, tap the box, type 'water the plants', tap the 'WHEN Today ˅' chip, tap Weekly, then Add.",
     "The When chip opens the door in place of the box: 'A day' (Today / Tomorrow / Pick a date), 'Or a rhythm' (Daily / Weekly / Every few days / Monthly) and Steps, with the summary top right. The summary reads 'Today · Weekly on <today's day>' and the Add button reads 'Add · Weekly on <day>' BEFORE the tap. After Add the task lands with the repeat glyph, the door resets to Today, and the panel stays up for the next line. Weekly is the chip, Weekly and Add: 3 taps beyond typing, never more.", "Both"),
    ("CAP-11", "Capture", "P1", "Text is never lost when the panel goes",
     "Tap the + pill, tap the box, type half a thought, then tap Close (and on a second try, the dimmed page above the panel). Tick a task. Tap the pill again.",
     "Close and the dimmed page both put the panel away and leave the words where they were. While words wait, the pill widens and shows '+' then three dots, and a screen reader hears 'Add a task. Your words are waiting'. Tapping the pill again shows the text still there. Nothing typed is ever lost by the panel closing, by selecting, or by a closed day.", "Both"),
    ("CAP-13", "Capture", "P1", "The panel rides ABOVE the keyboard, never under it",
     "On a real Android phone (app and browser separately), tap the + pill, then tap the box, and watch the panel as the keyboard rises. Type a line, tap the When chip, pick Weekly, tap the box again, Add. Repeat at a large font-size setting, on an iPhone in Safari (a normal tab), and in the iPhone home-screen app.",
     "The panel sits on top of the keyboard: its heading and Close, the box (shrunk to its minimum first), the tools row, When and Add all stay in view, so you can see what you type and reach Add without dismissing the keyboard. Opening When puts the keyboard away and shows the choices; tapping the box brings the keyboard back and folds When to its chip. In iPhone Safari, Safari's own small address label floats just above the keyboard: When and Add sit clear above it, never under it (Melroy's screenshot, 2026-09-27, before lib/safari-chrome). REGRESSION GUARD: a tester's v19 screenshots showed only 'ADD / Close' with everything else under the keyboard, because edge-to-edge Android ignores adjustResize and Chrome 108+ overlays the keyboard by default; the fix is the panel's own keyboard lift (native) + interactive-widget=resizes-content (web).", "Both"),
    ...
    ("TOD-30", "Today", "P2", "Repeating from the Menu comes back to the Menu",
     "Tap Menu, then the Repeating card. Tap '‹ Menu' (and, on Android, the system back). Then open Repeating another way (tap a repeating row's glyph, or any other door to it) and tap its back.",
     "The Menu's Repeating card opens the Repeating ROOM, a page of its own like Routines (it is no longer a drawer sliding over Today). Its top reads '‹ Menu', and '‹ Menu' or the system back return you to 'The rest of the house', where you started, never to a Today you never asked for. Opened any other way, the same room reads '‹ Today' and its back lands on Today.", "Both"),
    ("TOD-31", "Today", "P3", "The Menu page holds at large text and on a narrow phone",
     "Set the app's text size to its largest (Settings), open the Menu page. Then try a narrow phone (or a 320px-wide browser window).",
     "The cards stack one per row instead of two; every name and for-when line wraps in full, nothing clipped, no sideways scroll. The pictures keep their shape. An odd card at the end of the grid spans the full width rather than sitting alone in half a row.", "Both"),
    ("TOD-32", "Today", "P2", "The capture panel on a wide browser window",
     "On a desktop browser at full width, open Today, click the + pill, click the box, type, open When, pick Tomorrow, Add.",
     "The pill sits at the foot of the centred column, and the panel rises within the column's width (rounded top corners, the scrim behind it across the page). The cursor stays in the box after Add and does not drop out a second later (a regression found in the preview: the page's own scroll to a just-added row used to put the keyboard away on web). The browser's own focus rectangle never shows inside the rounded box; the box's accent edge is the focus sign.", "Web"),
    ("TOD-33", "Today", "P1", "Walking into a room: ‹ Menu, the picture, the title, the card's line",
     "From the Menu, open Routines, then Calendar, then Chart a course (AI on). In each, scroll down, then tap '‹ Menu'. Repeat with the system back gesture, in dark mode, at the largest text size, and with a screen reader on.",
     "Every room opens the same way: a fade and a small rise (a plain 90ms fade under reduced motion). At the top, '‹ Menu' in the accent, then the room's picture as a band (the same picture as its card, cropped to a wide strip, 104pt tall at EVERY text size, same image in dark), then the title in the serif and, as the subtitle, the card's for-when line word for word. The old subtitles are gone ('Gentle rituals. No streaks…', 'Everything you've actually finished.'). Below that, each room is exactly the room it was. Scrolling takes the band and title away while '‹ Menu' stays put. '‹ Menu' and the system back both return to the contents page. A screen reader lands on the title and hears it as a heading ('Routines, heading'), and never hears the band.", "Both"),
    ("TOD-34", "Today", "P2", "A back label names the place it returns to",
     "Open the Calendar from somewhere other than the Menu (the rested screen's link, a reminder, a pasted link), and Settings from a link on Today. Open the Ours pairing screen from the Ours list's 'Kept with …' line. Then open each from the Menu instead.",
     "From the Menu the top reads '‹ Menu' ('Back to Menu' to a screen reader) and goes back to the contents page. From anywhere else it reads '‹ Today' ('Back to Today') and the tap lands on Today, past anything in between. From the Ours list it reads '‹ Back' and returns to the list. A room reloaded on web with nothing behind it still has a working back.", "Both"),
    ("TOD-35", "Today", "P2", "Repeating from the Menu has the same room top",
     "Open Menu › Repeating. Read the top of the room, scroll the list, then go back with '‹ Menu' (and, on Android, with the system back). Repeat with a screen reader on, and with reduced motion on.",
     "It opens like every room: a fade and a small rise (a plain 90ms fade under reduced motion). At the top '‹ Menu' (clear of the status bar on a notched phone), then the washing-line picture as a band, 'Repeating' in the serif and 'For the bins, the bills, the plants'. The band and title scroll away with the list; '‹ Menu' stays. '‹ Menu' and Android's back both return to the contents page. A screen reader lands on the 'Repeating' heading when it opens. The not-a-notification line is no longer under the heading: it sits at the foot of the list (REP-04).", "Both"),
    ("TOD-36", "Today", "P1", "A done task wears one sign: the accent tick",
     "Tick a task on Today, a stepped task to its last step, a routine step, and a repeat in the Repeating room. Try two colour themes, light and dark.",
     "Each finished thing shows ONE sign: the round check fills with the theme's own accent (Dusk mauve, Sage green, Honey gold…), never a separate green, and its words turn soft ink but are NOT struck through. A stepped task's progress bar is the same accent. The tick stays readable on both schemes. Sage is left for celebration only (the logo, the Calendar's days, the rest-note), never on a control you tap.", "Both"),
    ("TOD-37", "Today", "P1", "Tuck: finished tasks fold into 'Done today'",
     "With 'Tuck away' on (SET-16), tick three tasks on Today one after another, including a repeat and a stepped task's last step. Watch each one. Then tap 'Done today · 3', untick one inside it, and tap the line again. Try with a screen reader, and in Quiet.",
     "Each ticked task stays in place for a beat so you SEE the tick, then folds away, and a quiet 'Done today · N' line on a hairline at the foot of the list counts them. Nothing else on the page changes: the gauge, Right now and close-the-day behave exactly as before. Tapping the line opens the finished tasks in place, each the same row with its accent tick; unticking one sends it back to its place in the list. The line closes again with a tap, and it is gone when nothing is finished. A screen reader hears 'Done. Tucked away with today's finished tasks.' when a row folds, and the line says whether it will show or hide them. Keep on Today (the default) leaves every finished task where it was, as before. The shared list is not affected.", "Both"),
    ...
    ("TOD-10", "Today", "P2", "Focus mode: pick-and-go",
     "Tap 'Focus on one thing' (the prominent entry above the list). On 'Which one?' pick a task; try 'Done' (complete), 'Choose another' (back to the list), and Exit. ALSO: with no pin but a HELD task (Hold me to it), tap Focus. And with BOTH a pin and a held task, tap Focus. THEN repeat with a LONG Today (15+ open tasks) and scroll the picker to its very last row.",
     "Full-screen single task, everything else hidden. Done completes it and returns to 'Which one?'; Choose another returns without completing; Exit closes. With a held task and no pin, Focus opens DIRECTLY on the held task (the contract is the obvious candidate; 'Choose another' remains the one-tap escape for working something else first, so the app suggests, never insists). With both, the PIN wins: it is the explicitly chosen priority and the paid anchor, and Hold never outranks it. When none left: 'That's everything for now.'. With a LONG Today the picker SCROLLS and every row is reachable, including the last, while Exit stays pinned in its corner and never scrolls away. EXIT MUST ALSO BE CLICKABLE, especially by mouse on web: the scroll fix originally rendered the full-bleed scroller AFTER the absolutely-positioned Exit, so the scroller stacked ON TOP of it and every click landed on the scroll surface instead; Focus could not be left by mouse at all (Melroy, web, 2026-07-25). The Exit now renders after the scroller with a zIndex, so it always wins the stack. Regression check: click Exit dead-centre with a mouse, on web, on a day with 6+ tasks. Regression guard: the picker used to be a centred non-scrolling column, so a list taller than the screen was clipped at BOTH ends and those tasks could not be selected at all (found 2026-07-25).", "Both"),
    ...
    ("TOD-16", "Today", "P1", "The Menu pill opens 'The rest of the house' (Today v3)",
     "On Today, tap the Menu pill top right. Read the page. Press and hold a card, then slide off it; then tap it. Try it signed out, signed in with no shared list, and with a live list. Try with AI off, and with Premium and without.",
     "The header shows the date and one pill (three dots and 'Menu'; Quiet appearance keeps just the word). It opens a PAGE, not a sheet, fading and rising in: '‹ Today' top left, Settings top right, the title 'The rest of the house' and the lead 'Today is the kitchen. These are the other rooms.' A wide Calendar card with its picture comes first ('For when your brain says you did nothing'), then a grid: Routines, Repeating, Ours and Chart a course, each with a picture and a for-when line. While a finger is on a card it dims (no ripple, no scale, no haptic); sliding off does nothing; lifting opens the room. Chart a course is there only with AI on, and wears the honey ✦ when you are not Premium; with AI off the three cards left put Ours across the whole last row. Ours reads 'A list kept with one person. Sign in first.' when signed out; with a live list it lands on Today's Ours tab (see OUR-123). The foot is a quiet 'Premium ›' row on a hairline, never the gradient. '‹ Today' returns to Today.", "Both"),
    ...
    ("RTN-01", "Routines", "P2", "Create and run a routine",
     ...
     "The routine appears grouped under its time-of-day with an 'N of M' progress. Tapping a step marks it done for today (the theme's accent tick, its words in soft ink, no strike-through) and updates the count. Calm, with no streak and no celebration pressure.", "Both"),
    ...
    ("RTN-08", "Routines", "P2", "A first visit: two explanations and two doors, nothing hard-coded",
     "With no routines and no rhythms, open Menu › Routines and read the whole page. Compare the two explanations with each other, and with the Repeating room's empty line (REP-09).",
     "Under the title: the routines explanation ('No routines yet. A routine is a few small steps you do together…'), then '+ New routine'. Then the Rhythms heading, its explanation ('A rhythm is a gentle reminder that repeats through the day, every few hours or at times you choose, like a glass of water or a stretch. It arrives as a notification and slips away on its own. It never lands on your list, so there's nothing to tick off and nothing to miss.'), and '+ New rhythm'. On web the 'Rhythm reminders arrive on your phone.' note sits under that explanation. The three rhythm presets (Water, Stand up, Meds) are gone from the room, and so is the Morning button under the empty line. The explanations read in ONE voice: the same size, the same soft ink, the same line spacing, in all three places.", "Both"),
    ("RTN-09", "Routines", "P2", "The Morning starter lives inside the new-routine form, only while you have none",
     "With no routines (a rhythm or two is fine), tap '+ New routine'. Tap 'Try a Morning routine'. Change a step, then Add routine. Open '+ New routine' again, then Cancel. Tap Edit on your routine. Finally remove the routine and open '+ New routine' once more.",
     "At the top of the form sits a paper strip: 'Not sure where to start?' and 'Try a Morning routine'. Tapping it FILLS the form (the name Morning, the Morning slot, four steps: a glass of water, any medication, a few minutes of movement, today's one thing) and saves nothing; everything stays editable and Add routine saves what you left. Once a routine exists the strip is gone from the form, and it never shows when editing. Remove every routine and the strip comes back, because the rule is 'while you have none'. Rhythms do not count as routines.", "Both"),
    ("RTN-10", "Routines", "P2", "The Water starter fills the new-rhythm form, and never saves on its own",
     "With no rhythms, tap '+ New rhythm'. Tap 'Water, around every 2 hours'. Change the active hours, then Add rhythm. Open '+ New rhythm' again.",
     "The same strip sits at the top of the rhythm form: 'Not sure where to start?' and 'Water, around every 2 hours'. Tapping it fills the form (the name Water, Every so often, every 2 hours, 9 am to 9 pm) and schedules nothing until Add rhythm, so the saved card carries the hours you chose. Once a rhythm exists the strip is gone. Stand up and Meds are not offered as suggestions anywhere; both can still be made in the form (RHY-04).", "Both"),
    ("RTN-11", "Routines", "P3", "After your first, no suggestions anywhere",
     "With one routine and one rhythm, read the whole Routines page, then open each '+ New' form. Try it in dark mode.",
     "Your routine card, '+ New routine', the Rhythms heading and explanation, your rhythm card, '+ New rhythm'. No suggestion strip in either form and no presets: nothing on the page is hard-coded. Dark mode is the same room on warm charcoal. The two strips are counted, names only, as routine.starter_opened and rhythm.starter_opened.", "Both"),
    # --- Rhythms (nudge-only recurring self-care) --------------------------
    ("RHY-01", "Rhythms", "P1", "Add a Rhythm from the Water suggestion, nudge-only, never a task",
     "With no rhythms yet, on the Routines screen scroll to Rhythms, tap '+ New rhythm', tap 'Water, around every 2 hours' in the 'Not sure where to start?' strip, then 'Add rhythm'. On Android, with the app closed, wait for a window hour.",
    ...
    ("RHY-04", "Rhythms", "P1", "Fixed-time Rhythm (meds): nudges at set clock times",
     "Tap '+ New rhythm', name it Meds, choose 'At set times', enter 8:00 and 20:00 (add a second time with '+ Add a time'), and tap 'Add rhythm'. Then Edit it: in the form switch between 'Every so often' and 'At set times', add a time (24-hour entry, e.g. 14:30), remove one, and save. On Android, wait for a set time to pass.",
     "The saved Meds Rhythm fires at 8:00 and 20:00 (the card reads 'At 8:00 am · 8:00 pm' in the device's time convention). Meds is no longer a one-tap preset (RTN-10): it is made here, in the form. The form is EITHER every-N-hours OR at-set-times, one mode at a time; times are 24-hour entry with a live preview that sorts them into clock order, duplicates collapse, at most 8 times, and saving with no valid time shows a calm 'Add at least one time.' hint, never a silent nothing. On Android each set time delivers one calm nudge (title = the Rhythm's name, DEFAULT importance, delivered 'around' the time, inexactly). Still nudge-only: never a task, never ticked, no streak anywhere, and Pause / Resume / Remove behave exactly as RHY-03.", "Both"),
    ...
    ("HLD-07", "Hold me to it", "P1", "The contract line lives INSIDE the held cell, and its ending plays there",
     "Hold a task (with a PINNED task present, and again without). Read the held row. Tap the line's 'I'm holding this one'. Then tap 'Let it go' on the line. Re-hold, then TICK the task and watch its cell. Finally: reopen the app on day two and re-read the line's words.",
     "The held row is ONE bordered cell (Melroy's device verdict 2026-08-30, superseding the separate strip): the task title on the first line, and inside the same hairline accent border a second line, indented to the title column, reading a small accent dot, 'I'm holding this one', and a quiet 'Let it go' at the right edge. The title appears exactly ONCE (the old strip repeated it directly above the row). The app is the subject: it is doing the holding, the user is not failing. The cell floats to the top of Today, BELOW a pinned row when one exists, and its hairline border reads visibly lighter than the pin's thick one. The words are IDENTICAL on day one and day nine: no counters, no elapsed time, no 'still'. Tapping 'I'm holding this one' opens Focus on that task; tapping the title line still toggles the task, and the row offers no Move up / Move down (the float would swallow the move). 'Let it go' releases in one tap with no dialog, same as the card's chip. On TICKING the held task, the second line turns sage for one last frame, STILL FLOATED AT THE TOP where the tick just happened: a tick and the italic 'You closed the one I was holding.', no button, resting about two seconds before fading (leaving instantly between frames under reduce-motion), announced once to screen readers, and only then does the row settle to its natural place. That frame is the feature's payoff: the contract ends in celebration, never in silence.", "Both"),
    ("HLD-08", "Hold me to it", "P2", "More and Select several stop reading as siblings",
     "Open any held card. Read the fold's More row and the shelf beneath it.",
     "The fold's disclosure says 'More' (bare, with a caret). The shelf's multi-select door now says 'Select several' (was 'Select more'), because with the fold's preview line gone the two 'more' labels sat almost adjacent and read as the same family (Melroy, 2026-08-23, round two). Renamed in all five locales: the collision existed in four of them.", "Both"),
    ("HLD-01", "Hold me to it", "P1", "A held task keeps knocking until it is done",
     "On a phone, hold a task, open the fold (Share to Ours · Remind me · Hold me to it · Pin), tap 'Hold me to it'. Note the affirm. Leave the app closed and watch notifications arrive at ~30m, ~90m, ~3h, ~6h. Read each notification's wording. Next morning check ~09:30.",
     "The affirm is calm ('Held. I'll keep at it, kindly.'). Knocks arrive on the ladder WITHOUT the app running (pre-scheduled, not chained). EVERY knock says exactly the same thing: the task title plus 'You asked to be held to this. Whenever you're ready.' No counters, no '3rd reminder', no guilt inflection: escalation of delivery, never of judgment. From the day after, one knock daily at 09:30 (not 9:00, which would stack on the daily reminder). The held row FLOATS to the top of Today, always BELOW a pinned row (the pin keeps its seat, thick border and pin mark; Melroy's 2026-08-23 device verdict superseded the earlier never-floats rule), wears a HAIRLINE accent border deliberately lighter than the pin's, and carries the contract line inside its own border (HLD-07). A screen reader hears 'holding you to this'. The affirmation is itself scheduling evidence: it renders only AFTER the knocks are scheduled, so seeing it means the contract is live (there is deliberately no user-visible notification count anywhere; the raw number was removed 2026-07-12 as overwhelming). On Android the knocks live on their own 'Hold me to it' channel, tunable separately from nudges.", "Both"),
    ("HLD-02", "Hold me to it", "P1", "Both exits are one tap and never questioned",
     "With a hold running: (a) tick the task done. Re-hold another task and (b) open its fold and tap 'Let it go'. After each, check the phone's pending notifications (or just wait past the next ladder step).",
     "Ticking the task ends the contract with silence in the NOTIFICATIONS: every scheduled knock is cancelled, including the daily follow-up (the proof is silence at the next ladder time, since no count is shown anywhere by design). The one on-screen word is the cell's sage closing line (HLD-07), which fades on its own. 'Let it go' ends it with 'Let go. That's that.' and NO confirm dialog: releasing is a state change, not a failure, and questioning it would be shame through the back door. No knock ever arrives after either exit.", "Both"),
    ("HLD-03", "Hold me to it", "P2", "One contract at a time: the swap asks, everything else never does",
     "Hold task A. Then try to hold task B. Read the dialog. Confirm the switch, then check that A's knocks are gone and B's are scheduled.",
     "The ONLY dialog in the whole feature appears: 'One thing at a time', naming A, offering Switch / Cancel. It exists because a swap is a choice between two contracts, not a confirmation of one. Switching releases A (no knock from A ever again) and holds B on a fresh ladder. Re-tapping hold on the ALREADY-held task is not a swap: it reads as 'Let it go' (the same control is the exit).", "Both"),
    ("HLD-04", "Hold me to it", "P2", "Quiet hours: an evening hold becomes one morning knock",
     "At ~22:00, hold a task. Check the phone's scheduled notifications (or wait until morning).",
     "Nothing fires between 21:30 and 08:30, ever: a reminder at 3am is not accountability and this audience already sleeps badly. All four ladder steps would land in the night, so they COLLAPSE into a single 08:30 knock rather than queuing four notifications in a row at breakfast, then the daily 09:30 follow-up takes over. The ladder is deterministic: the same hold time always yields the same schedule (the predictability the autistic side of the audience needs).", "Both"),
    ("HLD-05", "Hold me to it", "P2", "Web never offers it, and the contract survives the phone",
     "On the web app, hold a task and inspect the fold. On the phone, force-stop the app (or reboot the device), reopen, and check the pending notifications.",
     "The web fold has NO 'Hold me to it' (web cannot schedule native knocks, and we never render a control whose only outcome is an error). On the phone, the resilience sweep re-asserts the stored contract on every app open, healing OEM alarm wipes exactly as it does for Rhythms; a contract released on-screen also cleans up any orphaned knocks. The contract also survives a SLOW start: the ending choke point waits for the task list to actually load before judging the held task 'gone' (before 2026-08-30 a boot where the contract loaded first silently released it against the still-empty list; caught on web, latent on device). Telemetry: started/completed/released leave the device as bare counts with a coarse step bucket, no task text ever (see the privacy policy's feature-usage section).", "Both"),
    # --- The 2026-09-21 flow audit's fixes (22 confirmed defects, walked click-by-click) ----------
    ("AUD-01", "Audit fixes", "P1", "Web: every tick box, radio and chip speaks its state to assistive tech",
     "On doubledone.app with a screen reader (VoiceOver on macOS Safari, NVDA on Windows, or the browser's accessibility inspector): focus the Energy pills, tick a task, open the capture door and pick a When chip and a Repeat chip, open the held card's More fold.",
     "Energy pills announce 'radio, selected' for the active one (aria-checked); a task row announces 'checkbox, checked' after the tick and 'not checked' before; When / Repeat chips announce selected; the More row announces expanded when open. Before 2026-09-21 react-native-web dropped the accessibilityState object entirely, so none of this was audible on web (native was always fine). Every one of the app's ~40 state sites now uses the aria-* spelling, which React Native maps natively too.", "Web"),
    ("AUD-02", "Audit fixes", "P1", "Remove on the held card offers Undo, single and bulk",
     "Long-press a one-off task > Remove. Then Select several > pick two one-offs and a repeating task > Remove.",
     "A calm bar appears above the list: 'Removed. Undo' (or '2 removed. Undo' for the bulk case); tapping Undo brings the one-offs back exactly as they were, within about six seconds. The repeating task in the mixed selection was SKIPPED for today, not removed, so it is not in the undo (it returns tomorrow by itself). No confirm dialog anywhere: undo, never 'are you sure'.", "Both"),
    ("AUD-03", "Audit fixes", "P1", "A repeat follows the start date you picked",
     "Capture door: 'Pick a date' > a Friday > 'Weekly'. Read the summary and the weekday chips. Then 'Pick a date' > the 25th > 'Monthly'. Then toggle a weekday chip by hand and re-pick a date.",
     "'Fri 25 Sept · Weekly on Fr' with the Fr chip selected (NOT today's weekday); Monthly says 'on the 25th'. Once you have touched the chips yourself they are yours and a new date no longer moves them. Closing or adding resets everything. Before the fix the seeds were today's weekday and today's date, fixed at mount, so a task quietly repeated on a day the user never chose.", "Both"),
    ("AUD-04", "Audit fixes", "P2", "Premium is named before the bounce, and the paywall says why",
     "Free account: open the Right now caret and read 'Plan my day'. Tap it. Back. Then: Chart a course > Suggest steps; Settings > a locked colour theme; Settings > Quiet; long-press a task > Pin.",
     "Plan my day carries the honey ✦ mark (the same one Pin wears in the fold) and its screen-reader label says Premium. Every gate lands on the Premium page with one accent line under the title naming the feature that sent you ('Plan my day is part of Premium.', 'Chart a course is part of Premium.', 'Colour themes…', 'The Quiet look…', 'Pin…'). Opening Premium from the Menu or Settings directly shows no such line.", "Both"),
    ("AUD-05", "Audit fixes", "P2", "Copy that is true in every state",
     "Web: Menu > Repeating (read the grey note); Menu > Ours signed out (first sentence); footer 'Turn on daily reminder' with browser notifications denied. Both: Menu > Calendar, page to a past month and a future month with nothing finished; Menu > Routines, add a rhythm with zero routines; Close the day > Settle > Leave, then browser Back; fresh install onboarding, deny the reminder permission.",
     "The Repeating note on web no longer says 'choose Remind me' (there is no Remind me on web); it points only at the daily reminder in Settings. Ours signed out says 'You're not signed in here.' on web ('on this phone' on the phone). The denied-notifications line on web sends you to the browser's site settings, not the app's. Calendar: a past month reads 'A quiet month.', a future one 'Nothing here yet…', only the current month says 'so far'. Routines with only rhythms still shows the routines explanation, and '+ New routine' still opens with the Morning starter strip (RTN-09). Leaving Settle returns to the SAME Today (browser Back then leaves the app, it does not reveal a second Today). Onboarding after a denied permission shows a 'Try again' link under the note.", "Both"),
    ("AUD-06", "Audit fixes", "P3", "The polish batch",
     "Premium account: Menu (Chart a course row). Free account: Settings colour themes (Dusk's spoken label). Signed out: Premium page. Any: Repeating > Edit a repeat (the save button); Today with zero tasks at desktop width; long-press a task > More (spoken label with one fold row); Select several with one selected (spoken Remove label); Routines > remove a rhythm; fresh install reveal page; Select several with a 'big' task; Focus on one thing with exactly one open task.",
     "No Premium pill on Chart a course for a subscriber. Dusk is never announced as Premium and tapping it as a free user selects it rather than bouncing. On the web, the signed-out paywall shows the Monthly / Annual toggle and both prices (checkout still needs sign-in); on Android it shows no price at all (PREM-42). The edit-repeat button reads 'Save · Every Mon', with a verb. An empty day's note and '+ I also did that' sit above the dock, not under it. Spoken labels say '1 quieter action' and 'Removes 1 task'. Removing a rhythm says 'Rhythm removed.' and its name field is labelled 'Rhythm name'. The onboarding reveal rows show a small dot, not a tick-box replica. The 'big' chip stays visible in select mode. Focus with one open task opens straight on it, no 'Which one?'.", "Both"),
    ("TEL-01", "Telemetry", "P3", "Held-card usage reaches the events table as names only",
     "On a production (non-dev) build, use a few held-card tools: break a task down, pin one, set a reminder, open the More fold. Then query D1: SELECT event, COUNT(*) FROM app_events GROUP BY 1.",
     "Rows appear for breakdown.started, task.pinned, nudge.set, card.more and friends: bare names, no task text, no ids, no free values (any props on the wire are dropped server-side). card.more counts fold OPENS only, the denominator for 'how often does the fold hide something people need'. This set exists because the held card was about to be redesigned a third time on gut alone; every placement argument now has a number. Dev sessions never send (the __DEV__ gate), and the privacy policy's feature-usage section names this collection.", "Desktop"),
    ("REP-01", "Repeating", "P1", "Removing a repeating task from Today skips ONLY today",
     ...
     "Today manages days: the instance vanishes from Today with 'Skipped just for today. The repeat continues.', the series survives (still in the Repeating room), and it returns on its next due day. A mixed selection tombstones the one-offs and skips the recurring. Screen readers hear 'Remove today's instance of {title}' on BOTH the held card and the bulk bar. The recurring card also shows NO Tomorrow and NO Move to... (re-dating silently does nothing to a repeat, and an action that does nothing is worse than an absent one), but DOES offer 'Mark as a lot' (a chore can absolutely be a lot).", "Both"),
    ("REP-02", "Repeating", "P2", "Edit a series from the Repeating room",
     "Open Menu › Repeating, tap Edit on a series. Change the title, the cadence (daily / weekdays / every N), and the start date. Save.",
     "The room manages the series: the edit sheet prefills everything, saving updates the series in place (shape identical to a captured one), and Today reflects the new cadence. An empty title cannot save.", "Both"),
    ("REP-05", "Repeating", "P1", "Monthly: capture a task on a day of the month",
     "On Today, open the capture door (When · Repeating · Steps), type 'Pay the rent', tap Monthly in the Repeating row, and pick a day in the 1-31 grid. Read the Add button, then add.",
     "Monthly sits as a fourth chip beside Daily / Weekly / Every few days, and opens a calendar-shaped grid of 1 to 31 with TODAY'S day already chosen (so the common case is one tap, not two). The Add button names the outcome before the tap: 'Add · Monthly on the 15th'. The task lands repeating, appears in the Repeating room under 'Each month' as 'Every month on the 15th', and reaches Today on that day of every month.", "Both"),
    ("REP-06", "Repeating", "P2", "Monthly: a day some months do not have clamps, never skips",
     "Capture a monthly task on the 31st. Read the line under the grid. Then check the Calendar across February, April and a 31-day month.",
     "Choosing 29, 30 or 31 shows one quiet line: 'Short months use their last day.' It appears ONLY for those three days (nothing is said on the 3rd, which needs no explanation). The task then lands on 28 February (29 in a leap year), 30 April, and 31 in every month that has one. It is never SKIPPED: the months a skip would drop are exactly the ones a rent or a bill cannot afford to miss.", "Both"),
    ("REP-07", "Repeating", "P2", "Monthly: edit an existing series to and from monthly",
     "Menu › Repeating › Edit on a monthly series. Confirm the sheet opens with Monthly lit and the right day highlighted. Change the day, read the commit button, save. Then edit it again and switch it to Weekly.",
     "The sheet prefills from the series: Monthly is the selected chip and the stored day is the highlighted cell. Changing the day updates the commit button live ('Every month on the 1st'), which names the outcome rather than saying Save. Switching to Weekly drops the monthly day cleanly and the series repeats weekly from then on; a task is never both.", "Both"),
    ("REP-03", "Repeating", "P2", "Remove a series from the Repeating room (recoverable)",
     "In the Repeating room tap Remove on a series, then Undo within ~6 seconds.",
     "The series tombstones with a calm 'Repeating task removed. Undo' line under the heading, never a confirm dialog. Undo restores it fully (and re-syncs). Letting the bar lapse keeps it removed.", "Both"),
    ("REP-04", "Repeating", "P2", "The Repeating room says plainly that a repeating task does not notify you",
     "Open Menu, then Repeating, and read the note at the foot of the list (under the last card, or under the empty line). Follow what it tells you to do: hold a task on Today and look for 'Remind me', and open Settings and look for the daily reminder. Switch the app to another language and read it again.",
     "At the foot sits a quiet note in soft ink: 'A repeating task appears on Today when its day comes. It doesn't notify you. If you want to be told, hold the task on Today and choose Remind me, or turn on the daily reminder in Settings.' The word 'appears' is no longer in capitals, in any of the five languages. Both routes it names really exist and really do notify (on web the note leaves out Remind me, which web does not have, see AUD-05). It reads as a clarification, not a warning: nothing about it looks like an error. The reason it exists: the app calls a repeating TASK and a NOTIFICATION both a kind of 'reminder', so someone who sets a task to repeat can reasonably expect to be told when it is due, and then is not. That is our confusion to fix, not theirs to work out, and it is fixed by stating the limit where the expectation is formed rather than by renaming anything ('Repeating' is the right word).", "Both"),
    ("REP-08", "Repeating", "P1", "The Repeating room: a card per kind, the tick, Edit and Remove in place",
     "Make four repeats from capture's When: one daily, one weekly (a Thursday), one every 3 days, one monthly. Open Menu › Repeating. Tick the daily one, then untick it. Tap Edit on the weekly one, change its title, save. Remove the every-3-days one, then Undo. Go back to Today and look. Try it in dark mode and at the largest text size. If you have a repeating copy brought over from Ours, tick it in the room and then look at the shared list.",
     "One card per kind of rhythm, each headed in small bold capitals in soft ink: 'EVERY DAY', 'EACH WEEK', 'EVERY FEW DAYS', and 'EACH MONTH' (a monthly repeat gets its own card rather than going missing); a kind with nothing in it draws no heading and no empty card. Rows inside a card are split by hairlines: a ROUND tick (a task's circle, not a routine step's square), the title, and under it the cadence line ('Every day', 'Every Thu', 'Every 3 days', 'Every month on the 15th') with Edit and Remove on the same line, wrapping at large text rather than squeezing. Ticking fills the circle with the theme's accent and softens the title, never a strike-through; it is TODAY's tick, so Today shows it done too. Edit opens the same 'Edit repeating task' sheet as always, and the new title is on Today straight away. Remove shows 'Repeating task removed. Undo' under the heading, and Undo brings the series back whole. There is no '+ New' anywhere in the room: repeats are made from capture's When. A ticked Ours copy closes on the shared list too, exactly as a tick on Today does. Dark mode is the same room on warm charcoal.", "Both"),
    ("REP-09", "Repeating", "P2", "The Repeating room before your first repeat",
     "On a fresh install, or with every repeat removed, open Menu › Repeating.",
     "Under the title and 'For the bins, the bills, the plants', one line in the rooms' explanation voice (the same size, soft ink and spacing as the Routines explanations): 'No repeating tasks yet. When you add something, open When and choose how it repeats.' It names the capture door by its current label, When (it used to say 'When · Repeating · Steps'). Then the not-a-notification note (REP-04). No group headings, no empty cards, no button.", "Both"),
    ...
    ("AND-03", "Android", "P2", "Launcher long-press shortcuts",
     ...
     "'Brain dump' opens the app with the capture panel up and the box focused, ready to type. 'Focus on one thing' opens directly in Focus mode.", "Android"),
    ("AND-04", "Android", "P2", "Share text into DoubleDone (Android sheet + installed web app)",
     ...
     "Both paths behave identically: DoubleDone opens with the capture panel up on Today and the shared text already in its box, focused and ready to add, including when the panel was closed AND on a cold launch (regression: the parked seed was cleared before the box mounted, so a share only worked if the box was already open). The share is cleaned to ONE calm line: a browser's quoted-selection share ('\"Title\"\\n https://site#:~:text=...') lands as just the title, links are dropped when words exist, a bare link keeps the link with the highlight fragment stripped. Adding it makes a normal task; nothing is sent anywhere until you do. The web path needs the PWA installed (that is how browsers expose share targets).", "Both"),
    ("CAP-14", "Capture", "P1", "Share TEXT into DoubleDone on iPhone (1.5.1 or later)",
     "On an iPhone running 1.5.1 or later: in Notes select a line of text, tap Share, and pick DoubleDone. Then in Safari tap the page's Share button (a URL, no selection) and pick DoubleDone. Then repeat with DoubleDone fully closed (cold) and with it already open behind (warm).",
     "DoubleDone is in the sheet for BOTH a text selection and a page URL, and opens with the capture panel up on Today, its box focused and prefilled with one calm line (words kept, links dropped when words exist; a bare link kept with its highlight fragment stripped), cold and warm alike. Nothing is added until you tap Add. On 1.5.0 a text share never listed DoubleDone at all: the extension's default activation rule was URL-only (found 2026-09-25, fixed in app.json), so on 1.5.0 only the Safari URL share shows it.", "iOS"),
    ("CAP-15", "Capture", "P1", "The + pill at rest, and the panel rising (capture pill handoff)",
     "Open Today and look at the foot of the screen. Scroll to the end of a long list. Tap the pill. Then tap Close. Repeat with Reduce Motion on.",
     "At rest there is no bar at the foot of Today: one small rounded pill with a '+' and no word on it floats centred, 34 above the bottom edge, and rows scroll under it. The list always ends with one row's worth of empty space where the pill rests, so it never covers the last task. Tapping it raises a panel from the bottom (about 384 tall on a phone, never more than 55% of the screen) over a light scrim, headed 'Add to Today' with Close, and the pill fades as it rises. The keyboard stays DOWN until you tap the box. Close, the scrim, Android back and Escape (web) all put it away, and focus returns to the pill. With Reduce Motion there is no travel, only a quick cross-fade.", "Both"),
    ("CAP-16", "Capture", "P1", "Add keeps you composing and marks what landed",
     "Tap the pill, tap the box, type 'call the plumber', Add. Type 'water the fern', Add. Then Close. Repeat once with When set to Tomorrow, and once with a screen reader on (VoiceOver / TalkBack / a browser screen reader).",
     "After each Add the cursor stays in the box, so the next thing goes straight in, and the list moves so the newest row sits just above the panel. Each row added on Today carries a warm tint and a small 'just added' mark while the panel is up. Closing the panel clears the marks (a short fade, none under reduced motion). A screen reader hears 'Added.' after an ordinary add and 'Added. Tomorrow.' when it went somewhere else, on web as well as on the phone. A Tomorrow add lands under Later, not on Today, and the list stays still.", "Both"),
    ("CAP-17", "Capture", "P2", "Break it down keeps your words until you accept",
     "AI on. Tap the pill, type a big task ('sort out the garage'), tap Break it down. When the proposal appears, dismiss it. Then do it again and accept.",
     "Dismissing the proposal leaves 'sort out the garage' in the box, untouched, so nothing typed is lost to a change of mind. Accepting adds the steps, only then clears the box, and puts the panel away so the new steps are in view.", "Both"),
    ("CAP-18", "Capture", "P2", "Sort for me puts the panel away and says what happened",
     "AI on. Tap the pill, type three lines (one quick, one that can wait, one big), tap Sort for me.",
     "The lines are sorted onto the day, the panel closes, and the summary ('Sorted: N for today, M for tomorrow...') floats just above the pill on a soft card, where the eye already is. It never hides under the pill or the panel.", "Both"),
    ("CAP-19", "Capture", "P1", "Scrolling never uncovers the page under the app (Android Chrome)",
     "On an Android phone in Chrome (the browser, not the installed app), open Today with enough tasks to scroll. Scroll the list all the way down and back up, quickly and slowly. Then, with a few words waiting in the pill, pull down hard at the top of the list. Repeat once with the address bar tapped into view, and in a light and a dark theme.",
     "Chrome's address bar stays where it is while the list scrolls (as before v3). There is never a strip of white or any other colour below the app, never a glimpse of the shut panel ('Add to Today', Close), and the pill never jumps. Pulling down at the top does not reload the page, and the waiting words are still there. With the panel open and the keyboard up, When and Add stay above the keyboard. REGRESSION GUARD: once the docked footer became the floating pill, the list filled the screen, Chrome began hiding its address bar on scroll, and the app's height:100% left the freed 56px showing white and the shut panel (Melroy's screenshot, 2026-09-26). The fix is #root overflow:hidden in the shipped head (inject-web-meta), which stops Chrome treating the list as the page's own scroller, with overscroll-behavior, 100dvh, the theme's ground on the page and the sheet's clipped layer as backstops.", "Android"),
    ...
    ("ONB-01", "Onboarding", "P1", "Guided welcome on first run (9-screen)",
     "On a fresh install (or after clearing 'doubledone.onboarded.v1'), open the app. Walk Begin -> type a few lines -> Sort it for me -> Looks good -> Got it -> What you finish, you keep -> When you tick something off -> the shared list -> a Premium teaser -> Open Today. Try Back to a previous screen, and separately try Skip.",
     "Today redirects to the welcome exactly once: a 9-screen sequence with a quiet 9-dot progress, Skip on every screen but the last, and Back (top-left) from screen 2 on with the typed text intact. The dump is triaged in the AI's suggested order but EVERYTHING lands on Today, nothing is pushed to Later on first contact (before trust exists, a vanished line reads as data loss); the reveal teaches 'Everything starts on today. Move anything to tomorrow, or later, whenever you like.'; any big one is flagged 'Looks big, break it down?'; the safety-net and 'what you keep' screens follow as calm info; the penultimate screen is a calm one-screen teaser of what Premium adds (never a hard gate, fully skippable); Open Today saves the tasks and opens Today. Skip, or an empty Sort, leaves immediately, saving whatever was revealed. Never reappears once done. If the AI is slow or offline, everything lands on Today, nothing lost.", "Both"),
    ...
    ("ONB-06", "Onboarding", "P1", "The welcome asks what a finished task should do",
     "Walk the welcome (first run, or Settings -> Help -> See the welcome again). On 'When you tick something off', read both cards, Continue without touching anything, and finish. Then replay and choose 'Tuck away'. Try it with a screen reader.",
     "The step comes straight after 'What you finish, you keep.' Two cards: 'Keep on Today' (already chosen, 'Seeing it ticked is the reassurance.') and 'Tuck away' ('Out of sight, one tap away.'), each with a small picture of what it looks like, and the line 'You can change this any time in Settings.' Continue without a tap keeps things as they were. Choosing Tuck applies at once and shows in Settings. A screen reader hears two options in a group, with the chosen one checked. Skip and Back still work around it.", "Both"),
    ...
    ("MCP-03", "MCP", "P1", "list_today",
     ...
     "Returns your open tasks, each with an id. A repeating task due today appears once, marked (repeats), and drops after you tick or skip it. Today here is the UTC calendar day, so before about 10am AEST it can still be yesterday's (known; a timezone option is planned).", "Desktop"),
    ("MCP-04", "MCP", "P1", "complete_task on a one-off",
     "Ask the agent to complete one of the listed ONE-OFF tasks by id.",
     "'Marked it done. Nice.'; it reads as done in the app after the next sync (cold open).", "Desktop"),
    ("MCP-18", "MCP", "P1", "complete_task on a REPEATING task ticks one day, never closes it",
     "With a daily repeat on Today (it shows as (repeats) in list_today): ask the agent to complete it. Ask again. Then open the app (cold), check Today, the Repeating room and the Lookback. Then ask the agent to complete it for today's LOCAL date (say the date).",
     "First: 'Ticked for <date>. It repeats, so it will be back on its next day.' Second: 'Already ticked for <date>...'. In the app the task is still in the Repeating room, comes back on its next day, and the tick shows on <date> in the Lookback. Before about 10am AEST <date> is YESTERDAY's (the UTC day, named in the answer, known); the agent passing `day` with the local date lands it on today. It never ends the series: the repeat is still there tomorrow.", "Desktop"),
    ("MCP-19", "MCP", "P2", "update_task done:false un-ticks on the server only",
     "After MCP-18, on a phone that has already synced the tick: ask the agent to un-tick the repeat (update_task done: false). Then open the phone.",
     "The agent's answer says it un-ticked <date> on the server AND that a device that already synced the tick will bring it back. The phone does exactly that on its next open (ticks are kept on purpose). Nothing is lost, nothing shames.", "Desktop"),
    ...
    ("MCP-08", "MCP", "P1", "Disconnect AI connectors (immediate kill switch)",
     "With a URL-connected agent active, in the app go Settings -> AI -> open the 'AI agent access (MCP)' row -> Disconnect AI connectors. Then have the agent try a tool.",
    ...
    ("SET-05", "Settings", "P2", "Export your data",
     "In Settings -> Access & data, tap the 'Export your data' row (works with no account).",
    ...
    ("SET-12", "Settings", "P1", "Settings in five calm cards",
     "Open Settings from the Menu. Scroll the whole page, signed out and then signed in. Switch to the Quiet appearance and look again.",
     "'‹ Menu' at the top, then the unchanged title and line. Five sections, each a card under a serif heading: Comfort (Theme, Text size, Motion, Daily reminder with its time), Look (Colour theme and Interface, each marked '✦ Premium' in soft ink for a free user), AI (the status line, a 'Turn AI off ›' row, and, signed in, one 'AI agent access (MCP)' row), Access & data (signed in: Account with your email as its value; then Export your data, Privacy & data, Terms; signed in: Delete account and data last), then the Premium card, then Help (Send feedback, See the welcome again). Links are whole rows with a › in soft ink, never mauve text links. There is no Ours row any more. The footnote, version, update block and developer band are at the foot as before. In Quiet the cards lose their fill and border and the headings and hairlines do the separating.", "Both"),
    ("SET-13", "Settings", "P2", "AI agent access folds into one row, and nothing is lost",
     "Signed in, in Settings -> AI, tap 'AI agent access (MCP)'. Copy the token. Tap the row again.",
     "The row opens downward inside its card with a short fade (none under reduced motion): the explanation, the server URL, Copy my token (and the token itself once copied), the footnote, and Disconnect AI connectors in soft ink (a switch, not the next step). Tapping the row folds it away again. A screen reader hears it as expanded or collapsed.", "Both"),
    ("SET-14", "Settings", "P2", "Delete sits last, in brick, and confirms in place",
     "Signed in (throwaway account), in Settings -> Access & data, tap 'Delete account and data', then 'Keep my account'. Tap it again.",
     "Delete is the card's last row, in the muted brick of destructive actions, not the mauve of every other link. Tapping it opens the existing confirmation in place, with 'Keep my account' on the left and 'Delete' on the right in brick. Keep closes it with nothing changed. See DEL-01 for the delete itself.", "Both"),
    ("SET-15", "Settings", "P3", "The feedback form opens inside Help",
     "In Settings -> Help, tap 'Send feedback', type a line, Cancel. Open it again and send.",
     "The form opens in place inside the Help card (not two centred links far apart), and cancelling closes it without leaving the card. Sending behaves exactly as SET-06.", "Both"),
    ("SET-16", "Settings", "P1", "Finished tasks: Keep on Today or Tuck away",
     "In Settings -> Comfort, find 'Finished tasks'. Switch to 'Tuck away', go to Today, and tick something. Switch back to 'Keep on Today'.",
     "The row sits in the Comfort card with its hint ('Tucked away, they wait in a 'Done today' line at the foot of Today. Your Calendar keeps them either way.'). The choice takes effect at once, survives a reload, and is the same choice the welcome asked. Switching back puts every finished task back in the list where it was.", "Both"),
    ...
    ("PREM-01", "Premium", "P1", "Paywall renders",
     ...
     "The calm 'Keep every week' pitch, the A$5/mo price, the 1 -> 2 -> 4 tenure tiers. The feature list includes the personalisation pair (Quiet, the borderless look, and the seven colour themes) alongside Scan, Pin and the AI suite, so a user sent here from the Interface gate can see the thing they tapped.", "Web"),
    ...
    ("PREM-04", "Premium", "P1", "Entitlement flips to premium",
     ...
     "Premium shows active; the scrapbook is no longer monthly-gated (now weekly).", "Web"),
    ...
    ("PREM-15", "Premium", "P2", "Post-payment 'taking a while' recovery",
     ...
     "Instead of spinning forever it shows a calm message ('This is taking longer than usual. Your payment went through, give it a minute, then tap Refresh.') with a Refresh button and a pointer to send a note from Settings if it persists. Tapping Refresh re-checks and flips to premium once the entitlement lands. It never says the payment failed.", "Web"),
    ...
    ("PREM-25", "Premium", "P2", "An Apple subscriber on the web sees 'Apple handles it', not a portal error",
     ...
     "The app says the subscription is managed in Apple's settings on the device it was bought on. It does NOT 404 the Stripe billing portal or show 'Could not open the billing portal' (the bug the entitlement source column exists to fix).", "Web"),
    ...
    ("WN-01", "What's new", "P2", "The What's new card appears once, and is dismissible",
     "Open Today after a version whose WHATS_NEW id has bumped.",
     "A calm card at the top of Today, never a modal. The Ours entry runs to FOUR lines where earlier ones took two, which is the difference between a change and a surface: what it is, why it cannot become a scoreboard, why it cannot make your day heavier, and where to find it. One tap on Got it retires it forever.", "Both"),
    ...
    ("PREM-35", "Premium", "P1", "Every RevenueCat delivery lands in the rc_events audit log",
     "After a Worker deploy, make any Apple subscription change (or replay an event from the RevenueCat dashboard). Then run the three queries in docs/operations.md against the rc_events table.",
     "Every delivery is logged, including the ones the webhook deliberately ignores, each with an `outcome` saying why: applied, duplicate, transfer, unresolved-user, other-entitlement, stale-expiration, sandbox, no-op. `period_type` and `environment` are both captured, which is the whole point: 'was that a trial or a real charge, and was it even production' must be answerable from our own database in one query, without opening RevenueCat or App Store Connect. An ANONYMOUS purchase (no Supabase id) writes no entitlement row, as before, but now leaves a row here with user_id NULL, so a paying customer can never again be invisible to us. Breaking the table (or deploying before it exists) must NOT break the webhook: premium still gets granted, because the log fails silently by design.", "Desktop"),
    ("PREM-36", "Premium", "P1", "A TestFlight (sandbox) purchase never grants server-side Premium",
     "On a TestFlight build, buy Premium with a sandbox Apple ID. Then check the D1 entitlements table for that user, and the rc_events log. Also confirm what the app itself shows on that device.",
     "The webhook answers 200 with {received:true, sandbox:true} and writes NO entitlements row: sandbox money is not money, and before this guard a TestFlight build could grant itself real Premium. The delivery IS logged in rc_events with outcome 'sandbox', so testing stays visible rather than becoming a blind spot. The tester still SEES Premium on their device, because RevenueCat grants the entitlement locally and the client merges that over the server's answer, so testing the paid surface still works. A real PRODUCTION purchase is unaffected. And a sandbox event for someone who is already a genuine paying subscriber must leave them exactly as they were: ignoring, rather than writing premium=0, is what makes that true.", "Both"),
    ("PREM-37", "Premium", "P1", "An anonymous Apple subscriber gets the tenure they paid for",
     "On iOS, buy Premium WITHOUT signing in (the App Review 5.1.1 path). Wait, or use an account whose Apple subscription is months old. Open Premium and read the panel, then open the Scrapbook and check how many keepsakes are offered this week.",
     "The renewal date is shown ('Renews {date}'), where before an anonymous buyer saw 'You are Premium' and was never told when they would next be charged. And the weekly keepsake allowance scales with the REAL tenure of the Apple subscription (1/week, 2 after two months, 4 after six), not the 1/week floor. The old code merged only a boolean from the device, so `since` stayed null and every anonymous subscriber sat on 1/week forever: the same money for a quarter of the product. A lifetime or non-expiring entitlement must NOT be shown as a scheduled cancel.", "Both"),
    ("PREM-38", "Premium", "P2", "The free month is not sitting under the buy button on iOS",
     "On iOS, signed in, open Premium as a free user. Look at what is directly under 'Go Premium'. Then scroll to the bottom of the Apple block. Repeat signed OUT, and repeat on web or Android.",
     "Signed in on iOS the free-month offer is NOT beneath the buy button any more: it sits in its own zone at the bottom, above a rule, with 'No card, no charge, and it never becomes a subscription on its own.' under it. It is still THERE and still one tap, because hiding it would move a genuinely free offer somewhere an iPhone user would never look. Signed OUT it does not render at all on any platform, since starting a trial can only answer 'sign in' there and we never show a control whose only outcome is an error. On web and Android it stays exactly where it was, inline, because nothing on that screen can take money without a Stripe redirect. The copy no longer opens with 'Or' in any of the five locales, which only parsed as an alternative to whatever sat above it.", "Both"),
    ("PREM-39", "Premium", "P1", "An anonymous Apple purchase attaches itself when you sign in",
     "On iOS, buy Premium WITHOUT signing in. Confirm the D1 entitlements table has NO row for you. Then sign in inside the app. Wait a moment, then re-check D1, and open Premium on a SECOND device (or the web app) signed into the same account.",
     "Signing in calls RevenueCat's logIn, which aliases the Supabase id onto the anonymous customer, and the app then asks POST /apple/reconcile to look once. An entitlements row appears with source='apple' and the real period end, WITHOUT waiting for the next renewal webhook (which could be a month away, and might arrive as a TRANSFER we deliberately do not act on). Premium then works on the second device and on web. Before this, an anonymous buyer was invisible to us: no renewal date, no tenure-scaled keepsakes anywhere else, and no way for us to answer a support question about their own money.", "Both"),
    ("PREM-40", "Premium", "P1", "Reconcile cannot be used to claim somebody else's subscription",
     "Signed in as a free account with no Apple purchase, sign in and watch the network call to /apple/reconcile. Try calling it by hand with no token, with a junk token, and with a body naming another user's id or a product.",
     "The request carries NO body at all: the server uses the cryptographically verified `sub` from your own Supabase token and asks RevenueCat about exactly that App User ID, so a caller can only ever reconcile themselves, and only into a purchase RevenueCat already agrees is theirs. No token or a forged one answers 401 and writes nothing. A free account gets a calm {attached:false} rather than an error, because that is the ordinary answer for every web user, every Android user and every free iOS user. A sandbox/TestFlight subscription is refused, so a tester cannot mint real Premium. A RevenueCat outage answers 502, NOT {attached:false}, because reporting an outage as 'you have no subscription' would read as a downgrade.", "Desktop"),
    ("PREM-41", "Premium", "P3", "The white label on the premium gradient is readable",
     "Look at 'Plan my day', 'Suggest steps' and the Settings Premium card in light and dark.",
     "The white label and the Premium card's line read clearly over the gradient. Its middle stop was deepened (a white 17px label there was 3.4:1, now 4.6:1) with little visible change to the look.", "Both"),
    ("PREM-42", "Premium", "P1", "Android sells nothing: the Premium page (Path C)",
     "On an Android phone (the new build), open Premium signed OUT, then signed in as a FREE account, then from each gate: Scan, Pin, Plan my day, a colour theme, Quiet, a second keepsake in a month, and Your patterns. Repeat once with TalkBack on, listening to every label.",
     "The feature list and the gate's '<feature> is part of Premium.' line show, with 'The whole calm daily loop stays free, forever'. There is NO price or currency anywhere (no A$, no '/ month', no 'save 17%', no Monthly / Annual toggle), NO Go Premium, Subscribe or Upgrade, NO 'Billed securely via Stripe', and NO link to doubledone.app or Stripe. Signed out, the one button is 'Already have Premium? Sign in', which opens our own sign-in. Signed in and free, the only thing to take is 'Try Premium free for a month' with 'No card, no charge, and it never becomes a subscription on its own.' under it. TalkBack never speaks a dollar amount. This is the screen Google rejected 1.5.1 for (a fixed A$ price shown to a reviewer abroad).", "Android"),
    ("PREM-43", "Premium", "P1", "Premium bought elsewhere works on Android, and is managed elsewhere",
     "On the Android build, sign in as each of: a web (Stripe) subscriber, an Apple subscriber, a comp account, an account mid free-month, and a Stripe account in past_due. Open Premium each time.",
     "Every paying, comp or trial account shows Premium active, with its extras working. The Stripe subscriber sees one plain line, 'To change or cancel it, use the website where you subscribed, or email support@doubledone.app.', with no button and no link. The Apple subscriber sees the 'Apple handles it' line. The comp sees 'Your Premium is on us'. The free month shows 'Free until <date>, then back to the free monthly scrapbook. No charge, ever.' and no way to buy. The past_due account sees the paused notice with the support email and NO 'Update payment details' link. Nothing opens Stripe.", "Android"),
    ("PREM-44", "Premium", "P2", "The rest of the Android app carries no price",
     "On the Android build: walk the welcome to its Premium step; open Settings > Terms, and Settings > Privacy; with TalkBack, focus the Settings Premium card as a free account, and the Menu's Premium row as a subscriber; then, signed in as an account whose free month has already ended, tap 'Try Premium free for a month' and come back to Premium once more.",
     "The welcome's fine print reads 'It's in Settings whenever you're curious. No ads, ever.' (no A$5). The Terms' 'Premium and billing' says Premium is optional and not sold in this app, with no price, and Refunds names Stripe billing for people who subscribed on the website or in an earlier version of the Android app, with no price and no way to buy. Privacy says Stripe processes payments on our website. The Premium card is read as 'DoubleDone Premium' (never 'see plans'), and the Menu row for a subscriber as 'Active. Every extra is yours.' (never 'Manage your subscription'). That tap says 'You've already had your free month.' (with the usual 'The free monthly scrapbook is always yours.' under it) and no nudge to buy, the link itself goes, and on the next visit it is not offered again.", "Android"),
    ("PREM-45", "Premium", "P1", "The web and the iPhone still sell exactly as before",
     "After the Path C change: on the web, open Premium signed out and signed in; on an iPhone build, open Premium signed out.",
     "Web: the Monthly / Annual toggle and both A$ prices, Go Premium opening Stripe Checkout, and 'Billed securely via Stripe. Terms and refunds' are all there, unchanged. iPhone: the StoreKit prices in the viewer's currency, the buy button, Restore, and Apple's renewal terms are all there, unchanged. REGRESSION GUARD: Path C is a compile-time Android-only switch (lib/storefront.android.ts), so a web or iOS difference is a bug.", "Web"),
    ("PREM-29", "Premium", "P1", "The Android AAB does NOT contain Google Play Billing",
     "Build the Android AAB, unzip it, and grep for com.android.billingclient. Also confirm the app still builds and installs, and that Premium is NOT sold on Android (PREM-42).",
     "react-native-purchases (and Play Billing) is absent from the AAB: the expo.autolinking.android.exclude in client/package.json drops it. Android sells nothing (Path C, 2026-09-27): no price, no Go Premium, no Stripe. This is the regression guard on adding the native module.", "Android"),
    ...
    ("PREM-33", "Premium", "P1", "A dunning subscriber is never sold a second subscription",
     ...
     "Access has lapsed so the UPSELL panel shows, but with a calm notice at the top: 'Your Premium is paused while a payment retries. Updating your card brings it back. There is no need to buy again.' with an 'Update payment details' link that opens the Stripe portal (the customer id exists in this state). If the user taps Go Premium anyway, the server refuses with 409 billing_issue and the app explains the card is the fix, NEVER 'You're already on Premium' (false: they are not) and NEVER a second Checkout. REGRESSION GUARDS on the same server change (locked by stripe.test.ts): a LAPSED subscriber (status canceled, customer id kept) can still re-subscribe via a fresh Checkout, an abandoned checkout (incomplete) can still buy, and a trial still converts. The old guard keyed on premium && customerId, so a past_due user could mint a SECOND subscription whose webhook overwrote stripe_customer_id and orphaned the first: paying twice with one visible.", "Web"),

    ("PREM-34", "Premium", "P1", "An ANNUAL subscriber is warned a week before the charge, once",
     "Needs a Stripe test clock, because the whole feature is about a year passing. Put a test user on the ANNUAL price. Open the app once while the renewal is far away (this is what teaches the device the period is a year: it is only ever judged annual when first seen more than 40 days out). Advance the test clock to ~10 days before renewal and open Today again. Then open it several more times. Separately, repeat the whole thing with a MONTHLY subscriber, and again with an annual one that has been CANCELLED (cancel_at_period_end).",
     "The annual subscriber gets ONE new task, dated SEVEN days before the charge (not on the day of it, which would be a receipt rather than a warning), reading 'Your DoubleDone year renews on <date>. Nothing to do.' Re-opening the app does NOT add a second: it is remembered against that period, and the next one is only noticed after the period rolls. The MONTHLY subscriber never gets one, however many times the app is opened (twelve notices a year is nagging). The CANCELLED subscriber never gets one either, because nothing is renewing and saying otherwise to someone who has decided to leave is both false and unkind. A device that first opens the app INSIDE the last 40 days gets no notice that year, by design, and self-corrects the next. This is the only place the app writes a task nobody typed, so a false positive here is a spine violation, not a cosmetic bug.", "Both"),
    ...
    ("QUI-01", "Interface", "P1", "Premium unlocks the Quiet interface; free is gated",
     ...
     "Premium: the whole Today surface goes borderless. Card outlines drop, and every action that looked like a button or a link renders as plain text with no outline or fill: the header Menu pill (which keeps just the word 'Menu', no dots), 'Focus on one thing', 'Lighten today', 'Close the day', and 'Plan my day' (which in Standard is the filled premium gradient) all become plain text (accent for the AI / premium ones, soft ink for the calm structural ones). The energy / weight-of-today gauge is KEPT, not removed: it shrinks to a 3px hairline bar so the day's load still reads at a glance. The choice survives reload with 'Quiet' ringed. 'Standard' is the default and looks identical to the app today (gradient button, 6px gauge, outlined actions all return). Free: the block shows a 'Premium' tag and tapping 'Quiet' routes to the paywall with NO change applied ('appearance.locked' logged). A lapsed subscriber keeps whichever appearance they chose.", "Both"),
    ...
    ("QUI-06", "Interface", "P2", "Quiet draws Energy as three words",
     "Premium, Settings -> Look -> Interface -> Quiet. Open Today and change Energy between Low, Normal and High.",
     "No pills: Low, Normal and High are plain words, the chosen one in bold ink, the others in soft ink. Each still has a full 44pt tap area and nothing shifts when the choice changes. Standard keeps the soft pill.", "Both"),
    ...
    ("PIN-01", "Pin", "P1", "Premium: pin a task as the day's one thing",
     ...
     "The task gets a calm mauve PIN MARK (a drawn pushpin; the star it replaced read as favourite, not pin, per Melroy 2026-08-30, and a drawn SVG cannot hit the iOS missing-glyph failure the round-one flag did) and the thick accent border, and floats to the top of Today. 'Focus on one thing' then opens straight to it.", "Both"),
    ...
    ("PIN-03", "Pin", "P2", "Only one task is pinned at a time",
     ...
     "Task B is now pinned (pin-marked, floated to the top), and task A is no longer pinned. At most one pin ever exists.", "Both"),
    ("PIN-04", "Pin", "P2", "A pin syncs across devices",
     ...
     "The task is still pinned after the sync round-trip (the pin mark and float persist). Needs the pinned_at column applied in Supabase.", "Both"),
    ...
    ("OCR-01", "Scan", "P1", "Premium: the Scan button opens the camera",
     "As premium, tap the + pill and then Scan (camera) beside Speak in the panel.",
    ...
    ("OCR-02", "Scan", "P1", "Free: Scan routes to the upsell, never opens the camera",
     "As a free user, tap the + pill and then Scan in the panel.",
    ...
    ("CHART-02", "Chart", "P1", "Free: charting routes to the upsell, never plans",
     "As a free user, open the Menu, tap 'Chart a course', type a goal, pick a timeframe, tap 'Suggest steps', then come back.",
     "Before the tap the room already says '✦ Suggesting steps is part of Premium.' under the button, once and static. The tap routes to the Premium screen calmly (never a wall), and a 'premium.gate_hit' with reason 'chart' is logged. No plan is generated and nothing is added. Coming back, the goal and the chosen timeframe are still there, including after a purchase: on the phone the Premium panel's way out reads '‹ Chart a course', and on web a checkout round-trip brings the goal back the next time the room opens. Nothing is locked, blurred or priced.", "Both"),
    ...
    ("CHART-06", "Chart", "P2", "The timeframe stays named once the steps arrive",
     "Premium. Type a goal, pick 'In 2 months', Suggest steps. Then 'Not these, start over', pick no deadline, and suggest again.",
     "Once the steps arrive the chips step aside, and one quiet line above the heading keeps the choice named: 'By when · In 2 months' (then 'By when · No deadline'). The AI's heading is in italic serif; every step starts ticked, each with its minutes and a ✕. The button counts only what will be added.", "Both"),
    ("CHART-07", "Chart", "P2", "The steps show where they landed on Today",
     "Premium. Chart a course, accept the steps with 'Add N tasks'.",
     "You land on Today (never back on the spent proposal; back from Today does not return to it). The first step is on Today carrying the soft 'just added' tint, which fades away after a couple of seconds. The rest wait on their days.", "Both"),
    ("CHART-08", "Chart", "P3", "The small marks read on paper",
     "In light mode, open Chart a course, get a proposal, and look at an unticked box, a step's minutes, the ✕, the spread note and the note at the foot. Then look at a selected timeframe chip.",
     "All of them read clearly (they moved from the faintest ink to the soft ink). The instruction under the title is in full ink. A selected chip keeps its tinted fill and border, and its label is ink rather than the accent, which was too faint on the tint. In dark mode the selected label stays the accent.", "Both"),
    ...
    ("TOD-27", "Today", "P1", "Right now: one card near the top, a clock-set slot, the tools open downward, Energy pills",
     "Open Today in the MORNING (before 11), around MIDDAY (11-17), and in the EVENING (17+), reading the 'RIGHT NOW' card under Energy each time. Tap the caret at its right. Add and complete tasks and watch whether ANY control moves. Tap an unavailable (faded) tool. Set the Energy pills to High on a 7-task day and re-open the caret; set Low and read the gauge. Reload the app and check the pill. Turn AI off in Settings and re-open the caret. Also check with 0 tasks, in select mode, while typing in the composer, and after closing the day.",
     "ONE card sits under the Energy pills, near the top: an overline 'RIGHT NOW' + the tool that suits the HOUR (morning: Plan my day; midday: Focus on one thing; evening: Close the day), plus a caret ('All day tools'). The occupant is set by the CLOCK at open and never changes mid-session; it NEVER shows Lighten today. The caret opens the tools DOWNWARD inside the same card, in the same day order every time: Plan my day, Focus on one thing, Lighten today, Settle, Close the day, the current occupant tagged 'now', Plan my day wearing the honey ✦ when you are not Premium. The list moves down to make room and nothing covers it (no dim, no scrim). NOTHING in the card appears, vanishes or slides as tasks are added or completed: an unavailable tool stays in place at lowered contrast and tapping it explains in one plain line what the TOOL needs ('Plan my day needs two or more tasks.' / 'Lighten today is for a full day.'), never what the person lacks. The Energy pills (Low/Normal/High) sit under a small 'ENERGY' overline with the gauge above, on EVERY open day, including an empty one ('A clear day.'). A fresh day defaults to Normal. High re-scales what 'full' means, Low IS the low-capacity day, and the choice survives a reload but RESETS to Normal next morning. With AI OFF the card holds only the non-AI tools (AI tools are gone, not faded). In select mode the card RESTS (dimmed, does not respond) rather than vanishing, so the list does not jump. While the composer is open the card steps aside with the rest of the heading. On a closed day it is gone.", "Both"),
    ...
    ("OUR-120", "Ours", "P1", "Waiting-with-no-code is reachable, says so, and the re-mint is a real button",
     "Mint a code, leave Ours entirely (back to Today), then come back via the MENU's Ours row before anyone joins.",
     "The Menu lands on the shared LIST (usable solo while waiting), and the list's header line reads 'Waiting for your person - Get a new code' where 'Kept with {name}' will later sit; tapping it opens the pairing screen (both Melroy's C6 check and the field report hit this as a dead end before 2026-09-20). That screen is titled 'Waiting for your person', NOT 'Read this to your person' over an empty space (the code renders exactly once; the server keeps only a hash, so it can never be re-shown), and 'Get a new code' is a full primary button there, not a quiet text link. While the code IS still on screen, the old title and quiet link stay as they were.", "Both"),
    ("OUR-121", "Ours", "P1", "Whose email, and which code goes where",
     "Signed out: read the Menu's Ours line, open Ours, read the signed-out screen, tap Sign in and read the field. Then, on the sign-in code step, paste an Ours-style code (letters in it). Separately, signed in, open Ours 'I have a code' and type a 6-digit emailed sign-in code.",
     "The Menu's Ours line reads 'A list kept with one person. Sign in first.' (the old 'Needs your email, no password' invited typing the PARTNER'S email; a real user did exactly that on 2026-09-13 and it became their account). The Ours signed-out screen opens with the plain fact 'You're not signed in on this phone.' The sign-in email field is labelled 'Your own email', and the subtitle says sharing happens later, in Ours, with a code, never by typing the partner's email here. Letters in the 6-digit code field draw the calm hint that it will be an Ours invite code (a hint, never an auto-redirect or a cleared field). Six bare digits failing in the Ours join field add one line naming the emailed sign-in code as the likely mix-up, alongside the unchanged honest failure.", "Both"),
    ...
    ("OUR-20", "Ours", "P1", "The room opens from Today's heading and from the Menu",
     "With a live shared list, tap the list's name beside 'Today' in Today's heading. Then go back and reach it through Menu › Ours.",
     "Both land on the shared list itself, not the pairing screen, with a fade (a tab changing), not a sideways slide. The room's heading reads 'Today' and the list's name side by side, the name underlined as the current one, then the line 'Shared. Anything with a day reaches both your Todays on that day.' The date and the same Menu pill sit above it. 'Kept with {their name} ›' still goes to the pairing/management screen. Tapping 'Today' in the room's heading goes back to Today.", "Both"),
    ...
    ("OUR-33", "Ours", "P1", "A closed list is still readable, and read-only",
     ...
     "Every row is readable. There is no + pill at all. Ticking, renaming, removing and re-cadencing all do nothing, and each row says why. Each row still offers taking a copy for yourself.", "Both"),
    ...
    ("OUR-35", "Ours", "P1", "Reopening together is a handshake, never unilateral",
     "On a closed list, tap 'Reopen together\u2026' on ONE side only and STAY on that screen, hands off. Read the code to the second account. On that side, tap 'Got a code from them? Enter it to reopen.' (under the reopen hint on the closed-list screen) and enter it. Repeat with the closed list being the ONLY closed list on both accounts.",
     "The offering side gets a code and nothing reopens yet. The other side has a named door to the join field and entering the code there reopens the list for both, with everything as it was. It is never asked which kind of code it is, and it works when the closed list on screen is the only one (the D2 'handshake purgatory', 2026-09-22). Within about ten seconds, with nothing touched, the offering side's screen changes on its own: the code is gone and the live list shows 'They used the code. The list is open again\u2026'.", "Both"),
    ("OUR-122", "Ours", "P2", "Waiting screens look on their own, and 'Check now' is there for the impatient",
     "A: mint a pairing code and stay on that screen, hands off. B: join. Watch A. Then, on a closed list: A taps 'Reopen together\u2026' and stays; B redeems. Watch A. Then, on either waiting screen, tap 'Check now'.",
     "Both times A's screen changes on its own within about ten seconds: the pairing wait becomes '{name} joined.', and the closed list becomes the live list with the 'They used the code' line and the spent code gone. 'Check now' sits under the waiting text and under a minted reopen code, and does the same immediately. Once A has opened the list, the pairing screen underneath stops looking (nothing to see by eye; the rule is focus AND foreground AND under 30 minutes).", "Both"),
    ("OUR-123", "Ours", "P1", "The Ours card goes home rather than into a room",
     "With a live shared list, open the Menu and tap Ours. Then open the Menu from the Ours tab and tap Ours again. Then, with no shared list yet (signed in), tap Ours from the Menu.",
     "With a live list the contents page closes and you land on Today's Ours tab (the list's name underlined) in one short fade; back from there does not return to the contents page. From the Ours tab's own Menu, the Ours card simply goes back to it. With no list yet it opens the invite screen as a room: '‹ Menu' and the two-mugs picture as a band, then the invite screen as it was.", "Both"),
    ("OUR-124", "Ours", "P2", "The room opens with your list's own name, in any language",
     "With a named shared list (e.g. 'Just us') and the app set to Italian (or German, Spanish, French), tap the list's name in Today's heading.",
     "The room's heading shows your list's own name from the first frame, and the capture panel's heading reads 'Aggiungi a Just us'. It never shows the default ('La nostra lista') for a beat and then flips, which read as the app changing language. A list with no name shows the default in the app's language. The list's own name is shown as you typed it, in whatever language that was.", "Both"),
    ...
    ("OUR-47", "Ours", "P1", "Ours lives in Today's heading (Today v3)",
     "With a live list, look at the top of Today. Then leave the list (or sign out) and look again. With a screen reader, move through the heading.",
     "With a live list the heading reads 'Today' and the list's name side by side (its own name when it has one, else 'Ours'), 'Today' underlined as the current tab. With no live list there is only 'Today', and no empty space where Ours would be. A screen reader hears them as tabs, 'Today, selected' and '{name}'. The old Ours door card under the list is gone.", "Both"),
    ...
    ("OUR-55", "Ours", "P1", "Capture on Ours IS Today's capture",
     "Open a shared list and tap the + pill at the foot of the screen.",
     "The same pill and the same panel Today has, headed 'Add to {name}': the same box, the same Speak button (web), the same When chip and door, the same Add button naming its own consequence. Not a lookalike, the same component, so the two can never drift apart.", "Both"),
    ...
    ("OUR-58", "Ours", "P1", "The shared door offers When and Repeating, never Steps",
     "Open the door on Ours and compare it with the door on Today.",
     "Ours shows a When row and a Repeating row, and NO Steps row (a shared row has no slices field, and breaking a thing down is personal). The When row is deliberate: a dated or repeating shared row now reaches BOTH Todays, so a day here is a real act rather than a word that means nothing. See OUR-71 for what the When row rests on.", "Both"),
    ("OUR-59", "Ours", "P2", "Typed text survives the panel going",
     "Type half a sentence into the Ours panel, Close it, tick something, then tap the pill again.",
     "The pill shows '+' and three dots while words wait, and the half sentence is still there. The panel is always mounted, never unmounted. Only an Add resets the door; the words never reset on their own.", "Both"),
    ("OUR-60", "Ours", "P2", "The Ours panel rides above the keyboard",
     "On a real Android device, tap the + pill on the Ours list, tap the box and start typing.",
     "The panel and its Add button stay visible above the keyboard while you type. Nothing on this stack does this by itself (edge-to-edge Android ignores softwareKeyboardLayoutMode), so it is done by hand and must be re-checked on a device, never only in a browser.", "Android"),
    ("OUR-61", "Ours", "P3", "No AI on the shared capture",
     "Open the Ours capture panel with AI enabled in Settings, and type a long run-on line.",
     "No 'Break it down', no 'Sort for me', and no 'Tidy this into tasks': steps or rewrites a model proposes would land on a list another person reads, and pointing a model at somebody else's screen is a decision about them. Speak stays (on-device dictation, not a server call) and so does Scan, which reads a photo INTO the box for you to edit rather than authoring anything, and is the one AI action that belongs here. See OUR-62.", "Both"),
    ("OUR-62", "Ours", "P1", "Scan (premium) reaches the shared capture",
     ...
     "The panel rises with the words IN its box (keyboard down), not on the list. You read them, edit them, and Add them yourself. This is the surface Scan most belongs on: the most photographed list anybody owns is the one on the fridge, and that list is shared by definition.", "Android"),
    ...
    ("OUR-71", "Ours", "P1", "The shared capture rests on Anytime, and says what a day means",
     "Tap the + pill on Ours and read the line beside the When chip. Open the door. Pick Today, then Tomorrow, then Daily, reading that line each time.",
     "The When row reads Anytime / Today / Tomorrow / Pick a date, with Anytime selected, and the Add button says just 'Add'. Choosing Today makes it read 'Add · Today', because that is the notable case: it will appear on both your days. The line beside the chip says, in plain words, what the current choice means on a shared list (no day lives here; a day reaches both Todays; a rhythm repeats for both of you), before you add.", "Both"),
    ("OUR-112", "Ours", "P1", "A shared row's WHEN can be changed, and cleared",
     "In the room, hold a row. The door reads 'When…' with the row's current answer beside it. Open it. Set a date, Set. Reopen, pick a different date, Set. Reopen, tap Anytime, Set.",
     "All three land: date set, date CHANGED, and back to plain. Changing a date and getting back to a plain row were both impossible before this; remove-and-recreate was the only way out, and on a repeat that ends the series for both people. The button always names the outcome ('Set . Thu 20 Aug', 'Set . Anytime'), never 'Save'.", "Both"),
    ("OUR-113", "Ours", "P1", "Ending a rhythm does NOT mark the row done for both people",
     "Make a shared row repeat weekly and tick it on one of its days, on either device. Then hold it, open When…, tap the selected rhythm chip to release it, choose Anytime, and Set. Watch BOTH devices.",
     "The row goes plain and stays UN-ticked on both. It must not come back done. A repeat's ticks live per-date in a completion log and the row's own done flag stays false; the moment the recurrence goes, the merge would read that same log as a one-off's tick and mark the row finished, for both people, with a doneAt from whenever it was last ticked. Try it for a DATED ending too (release the rhythm, pick a day): that path feels least like an ending and is the easiest to get wrong.", "Both"),
    ("OUR-114", "Ours", "P2", "The day zone changes its question when a rhythm is alive",
     "In the room, hold a row and open When…. With no rhythm selected, read the label above the day chips. Now tap Daily. Read it again. Then tap Anytime.",
     "It reads 'A day' with no rhythm, and relabels to 'Starting' the moment one is selected, in the accent colour. The same chips now mean 'from when' rather than 'which day'. Anytime stays available throughout, and ONE tap on it ends the rhythm: the label goes back to 'A day' and the summary says 'The rhythm ends.' before anything commits. Tapping the selected rhythm chip releases it too, for anyone who finds that natural, and both routes land in exactly the same place.", "Both"),
    ("OUR-115", "Ours", "P2", "Setting a shared row to Today actually reaches both Todays",
     "In the room, hold a plain row, open When…, tap Today, and Set. Check BOTH Todays (look away and back on each).",
     "It appears under DUE TODAY on both. The line in the sheet reads 'Today . <the date>' on purpose: a shared row with no date reaches nobody, so Today has to write a real date rather than 'no date', which is the same bug that once shipped on the shared capture bar above this sheet.", "Both"),
    ("OUR-116", "Ours", "P3", "An idle Set does not wash the row on the other device",
     "On A, hold a row, open When…, change nothing, and tap Set. On B, leave the room and come back.",
     "No 'changed since you looked' mark on B. Every write stamps the row, and a fresh stamp is exactly what the other person's screen reads as a change, so a Set that altered nothing would send them hunting for something nobody did.", "Both"),
    ("OUR-118", "Ours", "P1", "A repeat you have already done today keeps its held card",
     "In the room, make a row repeat on TODAY's weekday, then tick it. Now hold it.",
     "The card still offers When… with its rhythm beside it, and it does NOT say 'Done is done.' A repeat is not a finished thing: it comes back, so the day you did it is exactly a day you might want to move it. Before this, ticking a repeat took its whole held card away until the next day, which put the When editor out of reach on the rows most likely to need it. A finished ONE-OFF still gets the minimal card, which is correct.", "Both"),
    ("OUR-117", "Ours", "P2", "The Repeating room's Edit sheet is untouched by the When editor",
     "Menu -> Repeating -> Edit on any series. Compare with before.",
     "Same sheet as always: title, the four cadence chips (Daily / Weekly / Every few days / Monthly), weekday toggles or the stepper or the 1-31 grid, 'Starting from' with its date, and a commit button naming the cadence. NO Anytime chip, NO day zone, and tapping the selected cadence chip does NOT release it. 'No rhythm' there would mean the entry should not exist, which is removal, and removal stays in the Repeating room.", "Both"),
    ("OUR-119", "Ours", "P2", "Monthly works on a shared row, and an out-of-date partner still sees the rhythm",
     "On Ours, hold a row and tap 'Repeat…'. Choose Monthly and a day. Read the summary, commit, then look at the row and at both Todays on that day. If a partner is on an older build (1.3.1 or earlier), have them look at the same row.",
     "Monthly is the fourth rhythm chip here too, from the same sheet the Repeating room uses, so a shared monthly row and a personal one are identical in shape. The summary reads 'Every month on the 15th · from {start}' and the commit button names it. The row lands on BOTH Todays on that day. A partner whose build predates monthly cannot compute the days, so their app shows the row with the plain-English line the writing client left alongside the rhythm rather than showing a repeating row with no rhythm at all. What it must never do is HIDE the row: one person seeing a task the other does not is the invisible disagreement this whole feature exists to prevent.", "Both"),
    ("OUR-111", "Ours", "P1", "A finished dated shared row leaves both Todays the next day",
     "Give a shared row today's date so it appears under DUE TODAY on both phones. Tick it on either phone. Look at both Todays now, then again tomorrow (or move a device's clock forward a day and reopen).",
     "Today: still there on BOTH, ticked, and un-tickable again, because ticking is the reversible act on this list. Tomorrow: gone from both. It must not accumulate. Before this rule the strip kept every dated row either of you had ever ticked, for good, on a screen excluded from the weight gauge and the close-the-day count, whose whole premise is that a day is finite. Check the other half too: an UNFINISHED row with a past date is still there tomorrow, which is correct and is not shaming.", "Both"),
    ...
    ("OUR-74", "Ours", "P1", "The heading's '!' says something changed since you looked, and clears when you look",
     "Device B changes something on the shared list. On device A, open Today WITHOUT visiting Ours. Then tap the list's name in the heading and come back. Also have device B delete a row and check again.",
     "A small tinted '!' sits beside the list's name in Today's heading, and a screen reader hears '{name}. 1 since you looked'. After visiting, it is gone. A deleted row never raises it. The mark is bounded by your own attention, not by the list's length, and it never shows a climbing number.", "Both"),
    ("OUR-75", "Ours", "P2", "Your OWN changes never raise the '!'",
     "On device A, add or tick several shared rows. Go to Today on the SAME device.",
     "No '!' in the heading. It uses the room's own wash arithmetic, so the heading and the room always agree about whose change it was.", "Both"),
    ("OUR-76", "Ours", "P2", "A first-ever visit raises nothing",
     "Pair two accounts, put rows on the list from device B, and open Today on a device A that has never opened the room.",
     "No '!'. There is no 'last looked' to measure against, and inventing one would open the app with a mark already on it.", "Both"),
    ...
    ("OUR-89", "Ours", "P2", "The pill steps aside for the select shelf",
     "Enter select mode in the room with a few words waiting in the capture panel.",
     "The pill is gone while selecting (the select shelf docks at the foot), and back when you cancel, still showing its waiting words. Two bottom-anchored surfaces competing for the thumb is how a calm screen stops being one, and you are not adding things while you are clearing things.", "Both"),
    ...
    ("OUR-105", "Ours", "P1", "First run introduces the shared list",
     "Reset onboarding (Settings, replay the first run) and step through it.",
     "An eighth step sits between 'keep' and 'premium', headed 'A list the two of you keep.' It says what arrives by itself, what deliberately does NOT, and that nothing keeps score. Its position matters: a FREE feature read immediately before the premium screen would be mistaken for part of the paid list.", "Both"),
