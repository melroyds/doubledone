...
CASES = [
    ...
    ("HLD-07", "Hold me to it", "P1", "The held strip: a promise being kept, and its ending",
     "Hold a task. Look at the head of Today's list (and again with a PINNED task present). Tap the strip's title. Then tap 'Let it go' on the strip. Re-hold, then TICK the task and watch the strip. Finally: reopen the app on day two and re-read the strip's words.",
     "While a contract is live, one slim tinted strip sits at the head of the list content, BELOW a pinned row when one exists (inside the scroll, never in the constant frame): a small accent dot, 'I'm holding this one' with the task title bold, and a quiet 'Let it go' on the right. The app is the subject: it is doing the holding, the user is not failing. It carries NO border and reads visibly lighter than the pinned row (float and border are Pin's paid signals), it never floats the task, never grows, and the words are IDENTICAL on day one and day nine: no counters, no elapsed time, no 'still'. Tapping the title opens Focus on that task. 'Let it go' releases in one tap with no dialog, same as the card's chip. On TICKING the held task, the strip turns sage for one last frame: a tick and the italic line 'You closed the one I was holding.', no button, resting about two seconds before fading (leaving instantly between frames under reduce-motion), announced once to screen readers. That frame is the feature's payoff: the contract ends in celebration, never in silence.", "Both"),
    ...
    ("HLD-02", "Hold me to it", "P1", "Both exits are one tap and never questioned",
     ...
     "Ticking the task ends the contract silently: every scheduled knock is cancelled, including the daily follow-up, with no comment about the hold at all (the tick's own celebration is the whole moment; the proof is silence at the next ladder time, since no count is shown anywhere by design). 'Let it go' ends it with 'Let go. That's that.' and NO confirm dialog: releasing is a state change, not a failure, and questioning it would be shame through the back door. No knock ever arrives after either exit.", "Both"),
