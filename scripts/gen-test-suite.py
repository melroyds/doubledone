...
CASES = [
    ...
    ("OUR-91", "Ours", "P1", "A copy says when its shared row was taken off the list",
     "Device B removes a row from Ours. On device A, look at the brought copy on Today.",
     "The copy STAYS (it is your task now) and carries a faint line: 'no longer on <list name>'. The deciding case is wasted effort: she removed milk because she already bought it, and a silent copy sends you to the shop.", "Both"),
    ("OUR-92", "Ours", "P1", "That line never says who, and never says why",
     "Read the line closely.",
     "It states the fact only. Not who removed it (no such data exists anywhere in this feature, and a bridge is where that temptation returns) and not why (removal could be done, not-needed or tidying, and the app cannot tell them apart).", "Both"),
    ("OUR-93", "Ours", "P2", "No line on a copy you already finished or removed",
     "Finish your copy, then have B remove the shared row. Then try it with a copy you removed yourself.",
     "No line in either case. You did the thing, so its origin's fate is not news, and a note there would read as a correction.", "Both"),
    ...
    ("OUR-97", "Ours", "P1", "A tick after midnight lands on the RIGHT day",
     "Open the room at 23:58 and leave it open. Tick a row at 00:02. Check the partner's device.",
     "Done on both, dated today. The room's date used to freeze at mount, so the completion log took yesterday's key and the partner's phone, asking about today, got false. Permanently: the merge is per-date.", "Both"),
    ("OUR-98", "Ours", "P1", "A tick made during a background sync is not undone",
     "Background and reopen the app (which triggers a settle), then immediately tick a row in the From Ours strip.",
     "It stays ticked. The settle now refuses to save a merge that predates any local write made while it was on the wire.", "Both"),
    ("OUR-99", "Ours", "P1", "A tick survives a device that has never opened the room",
     "On a device with no Ours cache, go offline, tick a brought copy, come back online, reopen the app.",
     "The tick reaches the shared list. It used to be lost forever: the only carrier pulled before writing, and nothing retried. The settle now carries it, with the copy's OWN completion time, so a deliberate un-tick on the other side still wins.", "Both"),
    ("OUR-100", "Ours", "P1", "A repeat is visible in the room EVERY day, tickable only on its own",
     "Make a weekly shared repeat. Open the room on a day it is not due.",
     "The row is there, with its rhythm in words, and its checkbox is inert with 'you can tick this on its day'. It used to vanish six days a week: unreachable to edit or remove, and a list of only repeats said 'Nothing here yet'.", "Both"),
    ("OUR-101", "Ours", "P2", "A closed list stops promising it keeps trying",
     "Open an archived list that has unpushed local changes.",
     "It says the list is closed, not 'Saved here, not there'. On a list where the write can never succeed, the second is a false hope on a screen a separated or bereaved person may keep for years.", "Both"),
    ("OUR-102", "Ours", "P2", "Setting a rhythm clears any date the row was carrying",
     "Give a DATED shared row a weekly rhythm, then read the row.",
     "One schedule, not two. The date is gone. Leaving it made the row answer differently depending on which code asked.", "Both"),
    ("OUR-103", "Ours", "P2", "Renaming inside the cadence sheet warns about the 500-character cap",
     "Open a row's rhythm sheet, paste a title over 500 characters, save.",
     "The trim notice appears BEFORE the save, the same as renaming on the row. It used to save silently and the server clamped it, so the words vanished a second later.", "Both"),
    ("OUR-104", "Ours", "P3", "No dead reorder rail on shared rows",
     "Hold a row in the room and in the From Ours strip.",
     "No up/down rail. Neither surface supplies those handlers, so it was two permanently inert controls on the screen briefed to be plainer than Today. Dim-in-place is for a control that is sometimes live.", "Both"),
    # --- Check for updates ------------------------------------------------------
