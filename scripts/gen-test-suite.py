...
CASES = [
    ...
    ("TOD-19", "Today", "P2", "Long titles scroll, even with a reminder or in select mode (Android)",
     "On Android, add a task with a very long title so it scrolls as a marquee. Set a reminder on it (tap-and-hold, Remind me). Watch the row. Then tap-and-hold to enter select mode and watch the scrolling titles.",
     "The long title scrolls smoothly and stays fully visible even with the reminder bell on the row (it never vanishes to a blank line). Entering select mode does not freeze or blank any scrolling title, including one carrying a reminder. With reduced motion on, a calm wrapped title shows instead.", "Android"),
