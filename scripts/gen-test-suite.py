...
CASES = [
    ...
    ("SB-08", "Scrapbook", "P2", "Share the keepsake (image only, never a link)",
     ...
     "What is shared is the keepsake IMAGE FILE itself (doubledone-week.jpg), never a public link and never the caption text (task-derived words are not silently attached). BOTH stored image shapes must share on Android: an older local (data:) keepsake AND an R2-persisted one (an https image, which once hit 'Sharing isn't available here', the 2026-07-12 device bug: the R2 image is fetched to the cache first, then the sheet opens with the picture). On a Web Share browser the sheet opens the same way; without it the jpeg simply downloads and one calm line says 'Saved. Share it anywhere you like.' Cancelling the sheet is quiet, no error and no line. A 'scrapbook.shared' event logs the how (shared / saved), never any content.", "Both"),
