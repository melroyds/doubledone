...
CASES = [
    ...
    ("TOD-07b", "Today", "P2", "First-run coachmark teaches the long-press",
     ...
     "A calm one-line hint sits above the list: 'Hold a task for more: break it down, make it tiny, move it, or mark it as a lot.' (reordered 2026-07-26 to match the design-1a held card, which leads with those four and folds Pin / Remind me behind More). 'Got it' dismisses it for good (survives reload). It never shows on an empty Today or the rested screen, and never returns once dismissed.", "Both"),
    ...
    ("ORD-01", "Today", "P2", "Free manual reorder from the held card (Move up / Move down)",
     ...
     "The held card shows one split row, '↑ Move up | ↓ Move down', FREE for everyone (this was born of users having NO ordering control: Pin is premium). Each tap moves the task ONE place and the card STAYS OPEN (deliberately not act-and-dismiss: three places is three taps, never three long-presses). The new order survives reload (stamped as manualOrder, local-only). Edges dim their button rather than hiding it, so the card never reshapes. The pin still owns the very top: nothing can be moved above a pinned task, and the pinned task itself offers no reorder.", "Both"),
