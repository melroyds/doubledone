...
- ✅ **Recurring "Starting from" at capture** (Melroy, shipped 2026-06-19): Daily / Weekly / Custom now take a start date (the `DatePicker`, past days disabled), so a habit can begin in the future; the Repeating drawer shows "· from {date}" until it starts. Decision-log 2026-06-19.
- Arbitrary-date one-off picker at **capture** for a single future task (distinct from the recurring start above). The `DatePicker` is now wired into the capture box, so this is a small follow-on. Trigger: you want to schedule a one-off for a specific far-off date at capture.
