...
**Queued behind it: the iOS IAP BUILD (the strategy call is MADE, 2026-07-15).** Apple IAP ships from iOS v1, parity-priced, absorbing Apple's 15% (see `decision-log.md`, 2026-07-15; this REVERSES the old 3.1.3(b) hide-everything plan). The dashboard side is done (RevenueCat project, both products, entitlement, offering). The CODE is at zero: `react-native-purchases` is not installed and there is no RevenueCat code anywhere. Until it lands, iOS is the worst of both paths, no IAP and no hiding, because `premium.tsx` still opens an external Stripe checkout on native (a 3.1.1 rejection on first submission). Live state and the ordered path: `docs/ios-iap-progress.md`.
