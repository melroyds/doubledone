...
- ◑ **Design overhaul, the "Dusk" system** (Melroy, 2026-06-18; in progress 2026-06-19). Melroy's design tooling produced the Dusk system (mockups in `docs/design/`). ✅ **Palette + system-following dark shipped:** dusky-mauve accent, periwinkle repeating, sage done, warm-paper light + warm-charcoal dark that follows `prefers-color-scheme` at launch (no setting). **Next:** the typography overhaul (Newsreader serif headings + Atkinson Hyperlegible body) and component polish to match the mockups (capture button, per-task accent dots). Then refresh the README screenshots (deferred #3 item).
