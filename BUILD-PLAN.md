...
- ◑ **Lock down the AI endpoints** (built 2026-06-19, deploy pending). The five AI routes now have a **CORS allowlist** (app origins only), an **Origin gate** (a disallowed browser origin → 403 before any Claude call), and a **per-IP Cloudflare rate limit** (`AI_LIMITER`, 30 req/60s) in `server/src/index.ts` + `wrangler.jsonc`. Server-only, no client change, nothing breaks for existing web/native users. Contract-tested (9 cases) and `wrangler deploy --dry-run` validated. **Remaining:** Melroy authorises `npm run deploy --workspace server`, then live-verify (no spend). See decision-log 2026-06-19.
...
- ◑ **Lock down the AI endpoints** (built 2026-06-19, deploy pending): CORS allowlist + Origin gate + per-IP rate limit now in `server/src/index.ts`; awaiting the production deploy + live verification. See the Backlog item and decision-log 2026-06-19.
