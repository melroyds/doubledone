...
**Premium.** Tier 1 (the feature flag, pin-a-task, the server-side requirePremium guard, OCR photo capture, and the owner comp allowlist) is **merged to main and live (2026-06-26)**. The native OCR viewfinder still awaits an EAS Android device test. **Tier 2 is built on the `premium` branch and staged for go-live (2026-06-26):** Richer Lookback insights, Chart a course, and Plan my order / sequencing, each gate-green and verified in the web preview, with the Premium page reframed to sell the whole suite. **Unlimited AI is deliberately HELD** for a product decision (building it overnight would bake an irreversible D1 schema around an unanswered question, for no user-visible value; see the decision-log). Go-live for Tier 2 is gated on Melroy's OK: deploy the Worker (the new `/lookback-summary`, `/chart`, `/sequence` routes) then merge `premium` to main. The full stack-ranked roadmap, the triggers, and the free-versus-premium wall are in [`docs/premium.md`](docs/premium.md).
