...
**Moat and AI data**
- ✅ **AI-call telemetry** (Melroy, 2026-06-18; code shipped, needs the manual deploy steps below). The Worker logs every AI call (decompose / strategise / triage) with its input, returned JSON, model, latency, token usage and ok/error to a Supabase `ai_calls` table, **insert-only RLS, NO user_id** (pseudonymous). Fire-and-forget via `ctx.waitUntil` so it never delays or breaks a user request; if the Supabase env is unset the Worker simply skips logging. Contract-tested (`server/src/telemetry.test.ts`), no live call in CI. **To go live, three manual steps:** (1) run the `ai_calls` migration in the Supabase SQL editor (`supabase/schema.sql`); (2) set `SUPABASE_URL` + `SUPABASE_ANON_KEY` as Worker secrets; (3) redeploy the Worker (`npx wrangler deploy` from `server/`). Hardening trigger (before public launch): the anon key can insert from anywhere, so add a Worker-side shared secret or rate limit, or move to Cloudflare D1 (the Worker-bound, no-public-write alternative).
