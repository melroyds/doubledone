...
**Moat and AI data**
- **AI-call telemetry** (Melroy, 2026-06-18; build next). Log every AI call (decompose / strategise / triage) from the Worker with its input, returned JSON, model, latency, and token usage. Recommended store: a Supabase `ai_calls` table with anon insert-only RLS and NO user_id, so it stays pseudonymous and separate from account identity (Cloudflare D1 is the cleaner-separation alternative). This is the foundation of the moat and of the prompt-tuning / cost loop. Must be disclosed in the privacy policy (the task text already goes to Anthropic; this retains it for product improvement). Trigger: now.
