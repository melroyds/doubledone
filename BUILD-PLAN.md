...
**Premium, in active development (the `premium` branch, not deployed).** Building the paid surface in isolation, and merging to main is the deploy. The premium **feature flag** is built (the gate every paid feature reads, with a dev override for local testing), and all three premium surfaces (Settings, Lookback's scrapbook gate, the Premium screen) now read it. Pin-a-task and the server-side requirePremium guard are both built (the guard cryptographically verifies the Supabase JWT via jose JWKS, ready for OCR to call). Next: OCR photo capture. The full stack-ranked roadmap, the engineering triggers, and the free-versus-premium wall are in [`docs/premium.md`](docs/premium.md).
