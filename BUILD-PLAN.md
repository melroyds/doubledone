...
- **One-off future date** (Melroy, 2026-06-19). Schedule a single task for a specific future day, distinct from the recurring "Starting from" that shipped. The `DatePicker` is already wired into the capture box, so the at-capture version is a small follow-on; the fuller version also sets / clears a future date on a task that already exists (likely via long-press). The task simply does not appear on Today until its day arrives. (Supersedes the earlier "arbitrary-date one-off picker at capture" note.) Trigger: you want to park a one-off for a specific far-off day.
- **Pushing a task to tomorrow** (Melroy, 2026-06-19). A calm per-task "not today" that rolls one task forward a day, the in-the-moment single-task version of what close-the-day already does for the whole list. Must read as a normal, guilt-free choice and never as failing the task (the never-shame rule): plain wording ("Tomorrow", not "Snooze" or "Postpone"), and no visible counter of how many times it has been pushed. Likely a long-press action beside step-back / remove. Trigger: working a day and wanting to defer one thing without opening close-the-day.
...
- ◑ **Disclose AI egress AND retention in-product** (largely done 2026-06-19): the `/privacy` policy states it plainly, two taps from Settings. **Optional remaining:** a calm one-liner right at the AI features (Break it down / Sort for me) for disclosure at the point of use. Trigger: before public launch.
