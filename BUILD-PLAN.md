...
**To do (triggers)**
- **Account and data deletion.** A signed-in user must be able to delete their account and all cloud rows (a hard delete, not just a tombstone). Trigger: before any public launch (Australian Privacy Principles, GDPR-style right to erasure). The `/privacy` policy already references this as "being added", so it is the natural next hardening item.
