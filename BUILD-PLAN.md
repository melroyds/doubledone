...
- **Multi-language support, starting Italian / Spanish / French** (Melroy, 2026-06-18). Make DoubleDone accessible beyond English (ParkProof shipped 9 locales; same instinct). Three parts: (1) **externalise every UI string** into locale files behind a small `t()` layer, with device-locale detection via `expo-localization` (a manual override lives in the Settings page); (2) **the AI in the user's language** — pass the locale to the Worker so decompose / plan / clarify / triage / strategise respond in it (the highest-value part, since the AI is the core of the app); (3) **the IT / ES / FR translations** themselves (AI-assisted draft, ideally native-speaker reviewed so the calm tone survives). Also localise date/number formatting (`'en-AU'` is hardcoded today in `lib/day` and the date picker). Sizable, cross-cutting; best as its own focused effort, and it pairs with the Settings page for a manual language switch. Trigger: Melroy wants it (now).
